...
void TATAKAE(int T)
{
    ...
    auto check = [&](int thres)
    {
        ...
        rep(i,1,n+1) rep(j,1,n+1) pre[i][j] = a[i][j] <= thres;    
        rep(i,1,n+1) partial_sum(all(pre[i]),pre[i].begin());
        rep(j,1,n+1) rep(i,2,n+1) pre[i][j] += pre[i-1][j];
        rep(i,1,n+1)
        {
            rep(j,1,n+1)
            {
