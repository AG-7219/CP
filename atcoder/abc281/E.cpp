...
void TATAKAE(int T)
{
    ...
    init(a,int,n,0)
    int ans = 0;
    multiset<int> useful, extra;
    rep(i,1,n+1)
    {
        if(useful.size() < k) useful.insert(a[i]), ans += a[i];
        else if(*useful.rbegin() > a[i])
        {
            int mx = *useful.rbegin();
            useful.erase(useful.find(mx));
            ans -= mx;
            extra.insert(mx);
            useful.insert(a[i]);
            ans += a[i];
        } 
        else extra.insert(a[i]);

        if(i>m)
        {
            int mx = *useful.rbegin();
            if(mx < a[i-m]) extra.erase(extra.find(a[i-m]));
            else 
            {
                ans -= a[i-m];
                useful.erase(useful.find(a[i-m]));
                ans += *extra.begin();
                useful.insert(*extra.begin());
