...
//dont use c++20, gl and hf
void TATAKAE(int T)
{
    get(int,n,m,e)
    int total = n+m+1;
    vector<int> parent(total), sizes(total,1), plants(total,0), cities(total,0);
    rep(i,1,total)
    {
        parent[i] = i;
        if(i<=n) cities[i] = 1;
    ...
    {   
        ...
        sizes[a] += sizes[b];
        parent[b] = a;
        if(plants[a] or plants[b])
        {
            if(!plants[a] and plants[b]) ans += cities[a];
            if(plants[a] and !plants[b]) ans += cities[b];
        }
        plants[a] += plants[b];
        cities[a] += cities[b];
        return ;
    ...
    vector<pii> edges(e+1); cin>>edges;
    get(int,q)
    vector<int> queries(q+1); cin>>queries;
    vector<bool> broken(e+1,false);
    rep(i,1,q+1) broken[queries[i]] = true;
    rep(i,1,e+1) if(!broken[i]) merge(edges[i].first,edges[i].second);
    vector<int> res;
    res.push_back(ans);
    repr(i,q,0)
    {
        merge(edges[queries[i]].first, edges[queries[i]].second);
        res.push_back(ans);
    }
    res.pop_back();
    reverse(all(res));
