...
void TATAKAE(int T)
{
    ...
    for(int r_mask=0; r_mask<(1<<h1); r_mask++)
    {
        if(bpc(r_mask) != h2) continue;
        for(int c_mask=0; c_mask<(1<<w1); c_mask++)
        {
            if(bpc(c_mask) != w2) continue;
            int r = 0;
            rep(i,0,h1)
            {
                if(!((r_mask>>i)&1)) continue;
                int c = 0;
                rep(j,0,w1)
                {
                    if(!((c_mask>>j)&1)) continue;
                    temp[r][c++] = a[i][j];
                }
                r++;
            }
