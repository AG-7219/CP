...
class DSU
{
    ...
    public:
        ...
        int get_size(int x) { return sizes[find_set(x)]; }
};
...
void solve(int T)
{
    ...
    repr(i,N-1,-1)
    {
        ...
        while(j<edges.size() and get<0> (edges[j])>i) 
        {
            auto [w,u,v] = edges[j];
            dsu.merge(u,v);
            j++;
        }
        for(auto& [ind,v]:queries[i])
        {
            ans[ind] = dsu.get_size(v);
        }
    }
    assert(!any_of(all(ans),f(i,i==-1)));
