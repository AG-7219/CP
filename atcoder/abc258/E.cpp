...
// dont use c++20 stuff, gl and hf.
void TATAKAE(int T)
{
    get(int,n,q,x)
    init(w,int,n,0)
    vector<int> pre(n+1); partial_sum(all(w),pre.begin());
    vector<int> visited(n+1,-1), aux;
    int i = 1;
    while(visited[i] == -1)
    {
        int temp = x%pre[n];
        ...
        if(temp)
        {
            if(pre[n] - pre[i-1] >= temp)
            {
                //we dont need rotation
                int lo = i, hi = n;
                while(hi >= lo)
                {
                    int mid = mid(lo,hi);
                    if(pre[mid] - pre[i-1] >= temp) ans = mid, hi = mid-1;
                    else lo = mid+1;
                }
                assert(ans!=-1);
                used += ans - i+1;
            ...
            else 
            {
                ...
                {
                    int mid = mid(lo,hi);
                    if(pre[n] - pre[i-1] + pre[mid] >= temp) ans = mid, hi = mid-1;
                    else lo = mid+1;
                }
                assert(ans!=-1);
                used += n-i+1+ans;
    ...
    int cycle_len = aux.size() - visited[i];
    debug(visited,aux,i,cycle_len)
    while(q--)
    {
        get(int,k)
        if(visited[i]>k) putn(aux[k])
        else 
        {
            k -= visited[i];
            k %= cycle_len;
            putn(aux[visited[i]+k])
        }
    }
}
