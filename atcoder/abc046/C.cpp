...
void solve(int T)
{
    get(int,n)
    vector<pii> ratios(n);
    rep(i,0,n) read(ratios[i]);
    ll lasta = ratios[0].fr, lastb = ratios[0].sc;
    rep(i,1,n)
    {
        ll lo = 1, hi = MX/(max(ratios[i].sc,ratios[i].fr)), ans = -1;
        while(hi >= lo)
        {
            ll mid = mid(lo,hi);
            if(mid*ratios[i].fr>=lasta and mid*ratios[i].sc >= lastb)
            {
                ans = mid;
                hi = mid-1;
            }
        ...
        assert(ans !=-1);
        lasta = ratios[i].fr*ans;
        lastb = ratios[i].sc*ans;
    }
    ll ans = lasta + lastb;
    putn(ans)
}
