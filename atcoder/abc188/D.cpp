...
void solve(int T)
{
    ...
    {
        cin>>v[i];
        aux.push_back(v[i].fr.fr); aux.push_back(v[i].fr.sc);
        aux.push_back(v[i].fr.fr+1); aux.push_back(v[i].fr.sc+1);
    }
    sort(all(aux));
    uniq(aux)
    map<int,int> inv;
    ...
    rep(i,1,aux.size())
    {
        int cur = (aux[i] - aux[i-1])*min(c,pre[i-1]);
        ans += (aux[i] - aux[i-1])*min(c,pre[i-1]);
        log(cur)
    }
    ans += min(c,pre[aux.size()-1]);
    putn(ans)
}
