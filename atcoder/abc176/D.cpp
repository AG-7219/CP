...
void TATAKAE(int T)
{
    get(int,h,w,sr,sc,dr,dc)
    vector<string> grid(h); for(string& row:grid) cin>>row;
    deque<pii> dq;
    vector<vector<int>> dist(h, vector<int>(w,oo));
    dist[sr-1][sc-1] = 0;
    dq.emplace_back(sr-1,sc-1);
    ...
    while (!dq.empty())
    {
        ...
        for(int i=0; i<4; i++)
        {
            ...
            if(nr<0 or nc<0 or nr==h or nc==w or grid[nr][nc]=='#') continue;
        ...
        for(int dr=-2; dr<3; dr++)
        {
            ...
            {
                int nr = r + dr, nc = c + dc;
                if(nr<0 or nc<0 or nr>=h or nc>=w or dist[nr][nc]<=dist[r][c]+1 or grid[nr][nc]=='#') continue;
                dq.emplace_back(nr,nc);
                dist[nr][nc] = dist[r][c] + 1;
            }
        }
    }
    int ans = dist[dr-1][dc-1];
    if(ans == oo) ans = -1;
    putn(ans)
