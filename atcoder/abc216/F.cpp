...
void TATAKAE(int T)
{
    ...
    init(a,int,n,0)
    init(b,int,n,0)
    vector<int> o(n+1); iota(all(o),0);
    sort_by(1+all(o), a[l] < a[r]);
    ...
    rep(i,1,n+1)
    {
        for(int sum=0; sum+b[o[i]]<=a[o[i]] and sum<=5000; sum++) ans = (ans + dp[sum])%M;
        for(int sum=5000; sum>=0; sum--) if(b[o[i]] + sum < dp.size()) dp[sum + b[o[i]]] = (dp[sum + b[o[i]]] + dp[sum])%M;        
    }
    putn(ans)
