...
void TATAKAE(int T)
{
    ...
    rep(i,1,n)
    {
        ...
        if(x[i] == 'A') player = 1;
        int val = (((s[i] - '0')%7) + 7)%7;
        val = (val * expm(10,i,7))%7;
        rep(j,0,7)
        {
            if(dp[i-1][j] == player) dp[i][j] = player;
            else if(dp[i-1][(j + val)%7] == player) dp[i][j] = player;
            else dp[i][j] = player^1;
        }
    }

    debug(dp)

    if(!dp.back()[0]) putn("Takahashi")
    else putn("Aoki")
