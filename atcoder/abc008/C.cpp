...
void solve(int T)
{
    ...
    rep(i,1,n+1)
    {
        for(int j=1; j*j<=v[i]; j++)
        {
            ...
            {
                divs[i] += cnt[j];
                if(v[i]/j != j) divs[i] += cnt[v[i]/j];
            }
        }
    }
    vector<ld> facts(101,1);
    rep(i,1,101) facts[i] = (facts[i-1]*i);

    auto nCr = [&](int p, int q)
    {
        return facts[p]/(facts[q]*facts[p-q]);
    };

    ld ans = 0;
    rep(i,1,n+1)
    {
        for(int nd=0; nd<=divs[i]; nd+=2)
        {
            int rem = n-1 - divs[i];
            for(int nnd=0; nnd<=rem; nnd++)
            {
                ld cur = nCr(divs[i],nd);
                cur *= nCr(rem,nnd);
                cur *= facts[nd+nnd];
                cur *= facts[n-(nd+nnd+1)];
                cur /= facts[n];
                ans += cur;
                debug(nd,nnd,v[i],cur)
            }
        }
    }
    precise(9)<<ans<<"\n";
