...
void TATAKAE(int T)
{
    ...
    vector<vector<int>> dp1(n+1, vector<int> (2,oo));
    vector<vector<int>> dp2(n+1, vector<int> (2,oo));

    dp1[n-1][1] = 0;
    dp1[n-1][0] = a[n-1];
    repr(i,n-2,0)
    {
        dp1[i][1] = min(dp1[i+1][1] + a[i], dp1[i+1][0]);
    ...
    repr(i,n-1,1)
    {
        dp2[i][1] = min(dp2[i+1][1] + a[i], dp2[i+1][0]);
        dp2[i][0] = dp2[i+1][1] + a[i];
    }

    debug(dp1,dp2)

    int ans = oo;
    amin(ans,a[n] + dp1[1][1]);
    amin(ans,a[1] + dp2[2][1]);
    putn(ans)
}
