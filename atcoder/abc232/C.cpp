...
{
    get(int,n,m)
    vector<pii> edges1(m+1), edges2(m+1);
    cin>>edges1;
    cin>>edges2;

    // debug(edges1,0,edges1.size())
    // debug(edges2,0,edges2.size())
    vector<int> p;
    p.push_back(0);
    ...
    do
    {
        ...
        {
            if(!i and !j) continue;
            if(find(all(edges2),make_pair(p[i],p[j]))==edges2.end() and find(all(edges2),make_pair(p[j],p[i]))==edges2.end())
            {
                log(i,j,p[i],p[j])
                flag = false;
                break;
    ...
    while(next_permutation(1+all(p)));
