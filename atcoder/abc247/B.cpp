...
void solve(int T)
{
    ...
    get(int,n)
    vector<pair<string,string>> names(n+1); cin>>names;
    rep(i,1,n+1)
    {
        bool flag1 = true, flag2 = true;
        rep(j,1,i)
        {
            if(names[j].first == names[i].first or names[j].second == names[i].first) flag1 = false;
            if(names[j].first == names[i].second or names[j].second == names[i].second) flag2 = false;
        }
        rep(j,i+1,n+1)
        {
            if(names[j].first == names[i].first or names[j].second == names[i].first) flag1 = false;
