...
void solve(int T)
{
    ...
    vector<int> prev_x(n+1,0), prev_y(n+1,0), prev_gtx(n+1,0), prev_smy(n+1,0);
    rep(i,1,n+1)
    {
        if(v[i] == x) prev_x[i] = i;
        else prev_x[i] = prev_x[i-1];

        if(v[i] == y) prev_y[i] = i;
        else prev_y[i] = prev_y[i-1];

        if(v[i] > x) prev_gtx[i] = i;
        else prev_gtx[i] = prev_gtx[i-1];

        if(v[i] < y) prev_smy[i] = i;
        else prev_smy[i] = prev_smy[i-1];
    }

    int ans = 0;
    rep(i,1,n+1)
    {
        int mn = min(prev_x[i],prev_y[i]);
        int mx = max(prev_gtx[i], prev_smy[i]);
