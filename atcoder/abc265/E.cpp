...
void TATAKAE(int T)
{
    ...
    //dp[i][j] denotes no of paths so far obtained by performing i ops of 1st type and j ops of 2nd type

    dp[0][0] = 1;
    for(int times=0; times<n; times++)
    {
        for(int first = times; first>=0; first--)
        {
            for(int second = times-first; second >= 0; second--)
            {
                int third = times - first - second;
                int i = first*a + second*c + third*e;
                int j = first*b + second*d + third*f;

                {
                    int nr = i + a, nc = j + b;
                    if(blocked.find({nr,nc}) == blocked.end()) dp[first+1][second] = (dp[first+1][second] + dp[first][second])%M;
                }

                {
                    int nr = i + c, nc = j + d;
                    if(blocked.find({nr,nc}) == blocked.end()) dp[first][second+1] = (dp[first][second+1] + dp[first][second])%M;
                }

                {
                    int nr = i + e, nc = j + f;
                    if(blocked.find({nr,nc}) != blocked.end()) dp[first][second] = 0;
                }
    ...
    {
        for(int second = 0; second <=n-first; second++)
        {
            ans = (ans + dp[first][second])%M;
        }
    }
    putn(ans)
}
