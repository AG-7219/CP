...
long long expm(long long a, long long b, long long m=M)
{
    long long res=1;
    while(b)
    {
...
void TATAKAE(int T)
{
    ...
    rep(i,0,n)
    {
        int val = s[i] - '0';
        int temp = 1;
        temp = (temp * expm(2,n-2))%M;
        temp = (temp * ((expm(5,n-i-1) - 1 + M)%M))%M;
        temp = (temp * expm(4,M-2))%M;
        temp = (temp + ((expm(10,n-i-1)*expm(2,i))%M))%M;
        val = (val * temp)%M;
        ans = (ans + val)%M;
    }
    putn(ans)
