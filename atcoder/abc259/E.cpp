...
{
    get(int,n)
    map<int,int> mx, cnt_mx;
    vector<int> primes[n], exponents[n];
    for(int i=0; i<n; i++)
    {
        get(int,m)
        primes[i].resize(m);
        exponents[i].resize(m);
        for(int j=0; j<m; j++) 
        {
            cin>>primes[i][j]>>exponents[i][j];
            if(exponents[i][j] > mx[primes[i][j]]) mx[primes[i][j]] = exponents[i][j], cnt_mx[primes[i][j]] = 1;
            else if(exponents[i][j] == mx[primes[i][j]]) cnt_mx[primes[i][j]]++;
        }
    }
    bool flag = false;
    int ans = 0;
    ...
    {
        bool flag2 = false;
        rep(j,0,primes[i].size())
        {
            if(exponents[i][j] == mx[primes[i][j]] and cnt_mx[primes[i][j]] == 1) 
            {
                ans++;
                flag2 = true;
                break;
