...
void TATAKAE(int T)
{
    ...
    while(i<s.length() and j<t.length())
    {
        if(s[i] != t[j]) putr("No")
        int itri = i, itrj = j;
        while(itri<s.length() and s[itri] == s[i]) itri++;
        while(itrj<t.length() and t[itrj] == t[j]) itrj++;
        if(itri-i > itrj - j) putr("No")
        if(itrj-j > itri-i and (itri-i == 1)) putr("No")
        i = itri;
        j = itrj;
    }
    if(i==s.length() and j==t.length())putr("Yes")
    putn("No")
