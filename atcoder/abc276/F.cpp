...
class segtree
{
    ...
    public:
        ...
        int query(int l, int r)
        {
            ...
            while(r>l)
            {
                if(l&1) ans = (ans + tree[l++])%M;
                if(r&1) ans = (ans + tree[--r])%M;
                l >>= 1, r >>= 1;
            }
            return ans;
        }
        ...
        void update(int ind, int upd)
        {
            ...
            {
                ind >>= 1;
                tree[ind] = (tree[2*ind] + tree[2*ind+1])%M;
            } 
        }
...
{   
    ...
    rep(i,1,n+1)
    {
        ans = (ans * expm(i-1,2,M))%M;
        int sum = (2*root.query(a[i]+1,N))%M;
        int cur = ((2*os.order_of_key({a[i],i}) + 1)*a[i])%M;
        ans = (ans + cur + sum)%M;
        ans = (ans * expm(i*i,M-2,M))%M;
        os.insert({a[i],i});
        root.update(a[i],a[i]);
        putn(ans)
    }
