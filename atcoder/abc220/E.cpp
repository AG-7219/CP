...
void TATAKAE(int T)
{
    ...
    //i is the number of bits in the rep of the ancestor
    rep(i,1,n)
    {
        //assume one end is the ancestor
        //the next d bits can be anything (you can go anywhere in the subtree)
        int ways = expm(2,i,M);
        if(n-i >= d) ans = (ans + (expm(2,d,M)*ways)%M)%M;

        //now we go left and right, 
        int mx = min(n-i,d-1);
        int mn = d - mx;
        if(d>=2 and mx>=mn) 
        {
            int temp = (ways*(mx-mn+1))%M;
            temp = (temp + M)%M;
            ans = (ans + ((temp*expm(2,d-2,M))%M))%M;
        }
    }
    putn(ans)
}
