...
void TATAKAE(int T)
{
    ...
    {
        vector<int> costs(n+1,0), visited(n+1,0);
        rep(i,1,n+1) for(int child:adj[i]) costs[i] += a[child];
        queue<int> q;
        rep(i,1,n+1)
        {
            if(costs[i] <= limit) q.push(i), visited[i] = true;
        }
        while(!q.empty())
        {
            auto node = q.front();
            ...
            {
                costs[child] -= a[node];
                if(!visited[child] and costs[child]<=limit)
                {
                    visited[child] = true;
                    q.push(child);
                }
        ...
        rep(i,1,n+1) if(!visited[i]) return false;
        return true;
    };
