...
int main(int argc, char const *argv[])
{
    ...
    while(t--)
    {
        get(n)
        sin(s)
        vector<int> pre(n,s[0] == 'W'), suf(n,s[n-1] == 'W');
        rep(i,1,n) pre[i] = pre[i-1]  + (s[i] == 'W');
        repr(i,n-2,-1) suf[i] = suf[i+1] + (s[i] == 'W');
        int ans = min(pre[n-2], n-1-suf[1]);
        rep(i,1,n-1)
        {
            ans = min(ans, pre[i-1] + n - (i+1) - suf[i+1]);
        }
        putn(ans)
    }
