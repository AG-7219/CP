...
void solve(int T)
{
    ...
    auto ub = [&](vector<int>& arr, int key)
    {
    ...
    {
        get(int,x)
        int ns = lower_bound(all(s),x) - s.begin();
        int nt = lower_bound(all(t),x) - t.begin();
        int ps = ub(s,x);
        int pt = ub(t,x);
        int ans = oo;
        if(ns<s.size() and nt<t.size()) amin(ans,max(s[ns],t[nt])-x);
        if(ps>=0 and pt>=0) amin(ans,x - min(s[ps],t[pt]));
        if(ps>=0 and nt<t.size()) amin(ans,2*(t[nt]-s[ps]) - max(t[nt]-x,x-s[ps]));
        if(pt>=0 and ns<s.size()) amin(ans,2*(s[ns]-t[pt]) - max(s[ns]-x,x-t[pt]));
        putn(ans)
    }
}
