...
void solve(int T)
{
    get(int,n)
    init(a,int,n,0)
    vector<int> aux(1+all(a));
    sort(all(aux));
    uniq(aux);
    debug(aux)
    segtree root(aux.size());
    int ans = 0;
    rep(i,1,n+1)
    {
        auto ind = lower_bound(all(aux),a[i]) - aux.begin();
        ind += aux.size();
        int cur = expm(2,i-1,M);
        cur = (cur * root.query(aux.size(),ind+1))%M;
        ans = (ans + cur)%M;
        root.update(ind,expm(expm(2,i,M),M-2,M));
        debug(i,root.tree)
    }
    putn(ans)
}
