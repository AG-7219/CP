...
void solve(int T)
{
    ...
    for(int& i:x) cin>>i;
    for(int& i:y) cin>>i;
    vector<int> pre1(n,0), pre2(m,0);
    partial_sum(all(x),pre1.begin());
    partial_sum(all(y),pre2.begin());
    debug(pre1,pre2)
    int a = 0, b = 0;
    rep(i,1,n) a = (a + ((i*x[i])%mod) - pre1[i-1]%mod + mod)%mod;
    rep(i,1,m) b = (b + ((i*y[i])%mod) - pre2[i-1]%mod + mod)%mod;
    int ans = (a*b)%mod;
    debug(a,b)
    putn(ans)
}
