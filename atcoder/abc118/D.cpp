...
{
    get(int,n,m)
    vector<int> a(m); for(int& i:a) cin>>i; sort(rall(a));
    vector<int> costs({INT_MAX,2,5,5,4,5,6,3,7,6});

    int nod = 0;
    for(int& digit:a) amax(nod, n/costs[digit]);

    vector<bitset<10001>> dp(nod);
    dp.push_back(1);
    repr(i,nod-1,-1) for(int& digit:a) dp[i] |= dp[i+1]<<costs[digit];
    ...
    for(int i=0; i<nod; i++)
    {
        for(int& digit:a)
        {
            if(costs[digit] <= n and dp[i+1][n-costs[digit]])
            {
