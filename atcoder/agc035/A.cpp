...
void solve(int T)
{
    ...
    ll xs = 0;
    map<ll,int> m;
    for(int i=1; i<=n; i++) xs^=v[i], m[v[i]]++;
    if(xs)
    {
        putn("No")
        return ;
    ...
    if(n%3)
    {
        if(m.size() == 1 and m.begin()->first == 0) putn("Yes")
        else putn("No")
    }
    else 
    {
        if(m.size()==1) putn("Yes")
        else if(m.size() == 2)
        {
            int a = m.begin()->second;
            int b = m.rbegin()->second;
            if(b>a) swap(a,b);
            if(a==2*(n/3) and b==(n/3)) putn("Yes")
            else putn("No")
        }
        else if(m.size() == 3)
        {
            bool flag = true;
            for(auto &[a,b]:m) if(b!=(n/3)) flag = false;
            if(flag) putn("Yes")
            else putn("No")
        }
        else putn("No")
