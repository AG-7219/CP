...
template<typename T1, typename T2> ostream& operator << (ostream& out, pair<T1,T2>& a) { out<<a.first<<" "<<a.second<<"\n"; return out; }
...
int main(int argc, char const *argv[])
{
    ...
    while(t--)
    {
        ...
        rep(i,0,62){
            ...
            mask%=mod;
            ans = (ans + ((((cnt)*(n-cnt))%mod)*mask%mod)%mod)%mod;
        }
        putn(ans)
    }
