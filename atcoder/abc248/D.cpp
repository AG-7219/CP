...
void solve(int T)
{
    ...
    vector<map<int,int>> inds(n+1);
    for(auto& [l,r,x]:queries) cin>>l>>r>>x, inds[l-1][x], inds[r][x];
    map<int,int> cnt;
    rep(i,0,n+1)
    {
        cnt[a[i]]++;
        for(auto& [key,val]:inds[i]) val = cnt[key];
    }
    for(auto& [l,r,x]:queries) putn(inds[r][x] - inds[l-1][x])
}
