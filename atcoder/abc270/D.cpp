...
{
    get(int,n,k)
    vector<int> a(k); for(int& stone:a) cin>>stone;
    ...
    for(int i=1; i<=n; i++)
    {
        for(int& stone:a)
        {
            if(i >= stone)
            {
                //player 1 removes `stone` and for the remaining pile, the other player removes dp[i-stone] so i remove (i-stone) - dp[i-stone]  
                amax(dp[i],stone + (i-stone) - dp[i - stone]);
            }
        }
    }
    cout<<dp[n]<<"\n";
