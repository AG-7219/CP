...
void TATAKAE(int T)
{
    ...
    map<int,int> inds;
    repr(i,n,0) inds[a[i]] = i;
    vector<int> pre1(n+1), pre2(n+1);
    rep(i,1,n+1)
    {
        if(inds.find(b[i]) == inds.end()) pre2[i] = n+1;
        else pre2[i] = inds[b[i]];
    }
    inds.clear();
    repr(i,n,0) inds[b[i]] = i;
    rep(i,1,n+1)
    {
        if(inds.find(a[i]) == inds.end()) pre1[i] = n+1;
        else pre1[i] = inds[a[i]];
    }
    rep(i,2,n+1) amax(pre1[i],pre1[i-1]);
    rep(i,2,n+1) amax(pre2[i],pre2[i-1]);
