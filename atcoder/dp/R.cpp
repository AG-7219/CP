        ...
        for (int32_t i = 0; i < rows; i++) {
            for (int32_t j = 0; j < a.cols; j++) {
                ...
                for (int32_t k = 0; k < cols; k++) {
                    res.mat[i][j] += (mat[i][k] * a.mat[k][j])%mod;
                    res.mat[i][j] %= mod;
                }
            }
        }
...
{
    get(int,n,k)
    matrix<int> mat(n,n); mat.fill(0);
  	for(int i=0; i<n; i++) for(int j=0; j<n; j++) cin>>mat.mat[i][j];
    mat = mat^k;
    int ans = 0;
    rep(i,0,n) rep(j,0,n) ans = (ans + mat.mat[i][j])%mod;
    putn(ans)
}
