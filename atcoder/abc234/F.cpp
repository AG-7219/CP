...
{    
    get(string,s)
    vector<int> facts(N,1), inv(N);
    rep(i,2,N) facts[i] = (facts[i-1]*i)%M;
    rep(i,0,N) inv[i] = expm(facts[i],M-2,M);
    vector<int> cnt(26,0); for(char c:s) cnt[c-'a']++;
    vector<int> dp(s.length()+1,0); dp[0] = 1;
    for(int cc:cnt)
    {
        for(int prev=s.length()-1; prev>=0; prev--)
        {
            for(int cur=1; cur<=cc and cur+prev<=s.length(); cur++) dp[prev+cur] = (dp[prev+cur] + (dp[prev]*inv[cur])%M)%M;
        }
    }
    int ans = 0;
    rep(i,1,s.length()+1) ans = (ans + (facts[i]*dp[i])%M)%M;
    putn(ans)
