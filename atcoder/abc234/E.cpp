...
void solve(int T)
{
    ...
    rep(i,1,10)
    {
        rep(j,-9,9)
        {
            if(i+j>=10 or i+j<0) continue;
            bool flag = true;
            string cur = "";
            cur.push_back(char(i + '0'));
            if(n.length() > 1) cur.push_back(char(i+j+'0'));
            rep(k,2,n.length())
            {
                int dig = i+ k*j;
                if(dig>=10 or dig<0) { flag = false; break; }
                cur.push_back(char('0'+dig));
            }
            log(cur,i,j)
            if(flag and stoll(cur) >= stoll(n) and ((ans == -1) or (stoll(cur) < ans))) ans = stoll(cur);
        }
    }
