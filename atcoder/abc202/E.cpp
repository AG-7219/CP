...
void dfs(int node, int par=-1, int depth = 0)
{
    ...
    for(int child:adj[node])
    {
        if(child!=par)
        {
            ...
            if(mx==-1 or sack[child]->size() > sack[mx]->size()) mx = child;
        }
    }
    if(mx!=-1) sack[node] = sack[mx];
    else sack[node] = new map<int,int>;
    (*sack[node])[depth]++;
    for(int child:adj[node])
    {
        if(child!=par and child!=mx)
        {
            for(auto [a,b]:*(sack[child]))
            {
                (*sack[node])[a] += b;
            }
        }
    }
    for(int i:qi[node]) ans[i] = (*sack[node])[queries[i].sc];
}
...
void solve(int T)
{
    ...
    get(int,q)
    rep(i,0,q) read(queries[i]), qi[queries[i].fr].push_back(i);
    dfs(1);
    rep(i,0,q) putn(ans[i])
}
