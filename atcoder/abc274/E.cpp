...
{
    get(int,n,m)
    vector<pii> coordinates(n+m);
    for(int i=0; i<n+m; i++) cin>>coordinates[i];

    auto dist = [&](const pii& src, const pii& dest)
    {
        return sqrt(pow(src.first - dest.first,2) + pow(src.second - dest.second,2));
    };

    vector<vector<ld>> dp((n+m), vector<ld>(1<<(n+m),oo));
    for(int i=0; i<n+m; i++) dp[i][1<<i] = dist({0,0}, coordinates[i]);

    for(int mask=0; mask<(1<<(n+m)); mask++)
    {
        ld speed = 1;
        for(int i=n; i<n+m; i++) if((mask>>i)&1) speed *= 2;
        for(int last=0; last<(n+m); last++)
        {
            if((mask>>last)&1)
            {
                for(int cur=0; cur<n+m; cur++)
                {
                    ...
                    amin(dp[cur][mask + (1<<cur)], dp[last][mask] + dist(coordinates[last],coordinates[cur])/speed);
                }
            }
        }
    }
    ...
    {
        int checker = (1<<n)-1;
        if((mask&checker) != checker) continue;
        ld speed = 1;
        for(int i=n; i<n+m; i++) if((mask>>i)&1) speed *= 2;
        for(int last=0; last<(n+m); last++)
        {
            if((mask>>last)&1)
            {
                amin(ans,dp[last][mask] + dist(coordinates[last],{0,0})/speed);
            }
        }
    }
    precise(10)<<ans<<"\n";
