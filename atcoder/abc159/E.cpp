...
{
    get(int,h,w,k)
    vector<string> grid(h); for(string& s:grid) cin>>s;
    ...
    for(int mask=0; mask<(1<<(h-1)); mask++)
    {
        bool valid = true;
        int cnt = bpc(mask);
        valarray last(0,bpc(mask)+1);
        for(int col=0; col<w; col++)
        {
            valarray cur(0,bpc(mask)+1);
            int j = 0;
            for(int row=0; row<h; row++)
            {
                cur[j] += grid[row][col] == '1';
                if(row != h-1 and ((mask>>row)&1)) j++;
            }
            if(cur.max() > k) valid = false;
            if((last + cur).max() > k) 
            {
                cnt++;
                last = cur;
