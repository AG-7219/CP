...
{
    get(int,d,g)
    vector<pii> pairs(d); for(auto& [p,c]:pairs) cin>>p>>c;
    int ans = oo;
    for(int mask=0; mask<(1<<d); mask++)
    {
        ...
        for(int i=0; i<d; i++)
        {
            if((mask>>i)&1) cur += pairs[i].fr, temp -= pairs[i].sc + (pairs[i].fr)*(100*(i+1));
        }
        for(int i=d-1; i>=0; i--)
        {
            if((mask>>i)&1) ;
            ...
            {
                if(temp <= 0) break;
                int problems = min((temp + 100*(i+1) - 1)/(100*(i+1)),pairs[i].fr);
                cur += problems;
                temp -= problems*(100)*(i+1);
            }
        }
        amin(ans,cur);
    }
