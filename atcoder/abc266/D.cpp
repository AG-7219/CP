...
{
    get(int,n)
    vector<vector<int>> cache(N, vector<int> (5,0));
    while(n--)
    {
        get(int,t,x,a)
        cache[t][x] = a;
    ...
    rep(i,1,N)
    {
        ...
        {
            //stay there
            amax(ndp[last], dp[last] + cache[i][last]);

            //move left
            if(last) amax(ndp[last-1], dp[last] + cache[i][last-1]);

            //move right
            if(last+1<5) amax(ndp[last+1], dp[last] + cache[i][last+1]);
        }
        dp = ndp;
    }
    putn(*maxv(dp,0,5))
