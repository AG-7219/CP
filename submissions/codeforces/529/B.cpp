...
{
    get(int,n)
    vector<pii> rects(n+1); cin>>rects;
    int mn = 0;
    rep(i,1,n+1) amax(mn,min(rects[i].fr,rects[i].sc));
    int ans = oo;
    for(int h=mn; h<=1000; h++)
    {
        ...
        rep(i,1,n+1)
        {
            if(rects[i].sc > h) cnt--, w += rects[i].sc;
            else if(rects[i].fr > h) w += rects[i].fr;
            else aux.push_back(rects[i].sc - rects[i].fr), w += rects[i].fr;
        }
        if(cnt < 0) continue;
        sort(all(aux));
        rep(i,0,min((int)aux.size(),cnt))
        {
            if(aux[i] > 0) break;
            else w += aux[i];
        }
