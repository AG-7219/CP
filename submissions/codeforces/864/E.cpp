...
{
    get(int,n)
    vector<array<int,4>> items(n);
    rep(i,0,n)
    {
        auto& [t,d,p,ind] = items[i];
        read(t,d,p);
        ind = i+1;
    } 
    
    sort_by(all(items),l[1] < r[1]);
    vector<vector<int>> dp(n,vector<int> (items.back()[1],0));
    vector used(n,vector<bool> (items.back()[1],false));
    for(int i=0; i<n; i++)
    {
        if(i) dp[i] = dp[i-1];
        auto& [t,d,p,ind] = items[i];
        for(int prev=d-t-1; prev>=0; prev--)
        {
            if(prev+t<dp[i].size())
            {
                if(amax(dp[i][prev+t],dp[i][prev] + p)) used[i][prev+t] = true;
            }
        }
    }
