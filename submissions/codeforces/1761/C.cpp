...
void TATAKAE(int T)
{
    ...
    function<void(int)> dfs = [&](int node)
    {
        ...
        {
            if(ans[child].empty()) dfs(child);
            for(int val:ans[child]) ans[node].pb(val);
        }
        sort(all(ans[node]));
        uniq(ans[node]);
        ans[node].pb(cnt++);
    ...
    rep(i,1,n+1)
    {
        if(ans[i].empty() and !in[i]) dfs(i);
    }
    rep(i,1,n+1) 
    {
        cout<<ans[i].size()<<" "; logarr(ans[i],0,ans[i].size())
    }
}
