...
void TATAKAE(int T)
{
    ...
    vector<vector<vector<bool>>> dp(26,vector<vector<bool>> (n+1, vector<bool> (n+1,false)));
    for(int i=0; i<topo.size(); i++)
    {
        for(int j=0; j<=i; j++)
        {
            int node1 = topo[i], node2 = topo[j];
            for(char c = 'a'; c<='z'; c++)
            {
                for(auto& [child,ec]:adj[node1])
                {
                    if(ec >= c and !dp[ec-'a'][node2][child]) dp[c-'a'][node1][node2] = true;
                }
            }
            for(char c = 'a'; c<='z'; c++)
            {
                for(auto& [child,ec]:adj[node2])
                {
                    if(ec >= c and !dp[ec-'a'][node1][child]) dp[c-'a'][node2][node1] = true;
                }
            }
        }
    }

    for(int i=1; i<=n; i++) for(int j=1; j<=n; j++) cout<<(dp[0][i][j] ? 'A' : 'B')<<(j==n ? "\n" : "");
}
