...
void TATAKAE(int T)
{
    ...
    vector<int> persons;
    rep(i,0,n) if(a[i]) persons.push_back(i);
    if(persons.empty()) putr(0)
    vector dp(n,vector (persons.size(),oo));
    for(int i=n-1; i>=0; i--)
    {
        for(int j=(int)persons.size()-1; j>=0; j--)
        {
            if(!a[i])
            {
                if(i+1<n and j+1<persons.size()) amin(dp[i][j],abs(i-persons[j]) + dp[i+1][j+1]);
                else if(j == persons.size()-1) amin(dp[i][j],abs(i-persons[j]));
            }
            if(i+1<n) amin(dp[i][j],dp[i+1][j]);
        }
    }
    putn(dp[0][0])
}
