...
{
    get(string,s)
    vector<int> pre(s.length(),s.front() == '0'), suf(s.length(),s.back() == '1');
    rep(i,1,s.length()) pre[i] = pre[i-1] + (s[i] == '0');
    repr(i,(int)s.length()-2,-1) suf[i] = suf[i+1] + (s[i] == '1');
    debug(pre,suf);
    int ans = 0;
    rep(i,0,s.length())
    {
        bool flag = true;
        if(i and pre[i-1]) flag = false;
        if(i+1<suf.size() and suf[i+1]) flag = false;
        ans += flag;
    }
    putn(ans)
}
