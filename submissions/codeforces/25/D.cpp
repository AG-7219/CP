...
class DSU
{
    ...
    public:
        ...
        bool iscyclic(int id) {return cyclic[find_set(id)];}
};

void solve(int T)
{
    get(int,n)
    vector<pii> dest, build;
    DSU dsu(n);
    rep(i,1,n)
    {
        get(int,a,b)    
        if(!dsu.merge(a,b)) dest.pb(a,b);
    }
    int last = -1;
    rep(i,1,n+1)
    {
        if(dsu.find_set(i) == i)
        {
            if(last!=-1) build.pb(last,i);
            else last = i;
        }
    }
    assert(build.size() == dest.size());
    ...
    putn(ans.size())
    for(auto& [i,j,k,l]:ans) putn(i,j,k,l)
}
