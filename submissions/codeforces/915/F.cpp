...
void TATAKAE(int T)
{
    ...
    vector<int> o(n); iota(all(o),1);
    sort_by(all(o), a[l] < a[r]);
    int ans = 0;
    DSU mn(n), mx(n);
    for(int node:o)
    {
        ...
        {
            if(a[node] >= a[child]) ans += a[node]*mx.merge(node,child);
        }
    }
    cerr<<ans<<"\n";
    sort_by(all(o), a[l] > a[r]);
    for(int node:o)
    {
        for(int child:adj[node])
        {
            if(a[node] <= a[child]) ans -= a[node]*mn.merge(node,child);
        }
    }
    cout<<ans<<"\n";
}
