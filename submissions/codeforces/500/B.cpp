...
{
    get(int,n)
    vector<int> p(n); for(int& val:p) cin>>val;
    dsu ds(n);
    for(int i=0; i<n; i++)
    {
        get(string,s)
        for(int j=0; j<n; j++) if(s[j] == '1') ds.merge(i,j); 
    }
    auto components = ds.groups();
    for(auto group:components)
    {
        vector<int> vals;
        for(int ind:group) vals.push_back(p[ind]);
        sort(all(vals));
        for(int i=0; i<group.size(); i++) p[group[i]] = vals[i];
    }
    logarr(p,0,n)
}
