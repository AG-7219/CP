...
{   
    ...
    sort_by(all(liked),l.sc < r.sc);
    sort_by(all(notliked),l.sc < r.sc);
    debug(liked,0,liked.size())
    debug(liked,0,notliked.size())
    vector<int> ans(n+1);
    rep(i,0,notliked.size())
    {
        ans[notliked[i].fr] = i+1;
    }
    rep(i,0,liked.size())
    {
        ans[liked[i].fr] = i+1+notliked.size(); 
    }
