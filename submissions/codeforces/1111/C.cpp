...
{
    get(int,n,k,a,b)
    vector<int> pos(k); for(int& x:pos) cin>>x;
    sort(all(pos));

    function<int(int,int,int,int)> rec = [&](int l, int r, int x_l, int x_r)
    {
        int ans = 0;
        
        //burn the current base
        if(r>=l) ans = b*(r-l+1)*(x_r-x_l+1);
        else ans = a;

        //divide in two and solve 
        if(x_r - x_l >= 1 and r>=l)
        {
            int mid = (x_l + x_r)/2;
            amin(ans, rec(l, (upper_bound(all(pos),mid)-pos.begin())-1, x_l, mid)
            + rec( lower_bound(all(pos),mid+1) - pos.begin(), r, mid+1, x_r));
        }

        return ans;
    };

    putn(rec(0,k-1,1,1<<n))
}
