...
void TATAKAE(int T)
{
    ...
    partial_sum(rall(a),mn1.rbegin(),g(l,r,min(l,r)));
    rep(i,1,n+1)
    {
        int rem_len = n - (i-1);
        debug(i,suf[i],a[i],rem_len)
        if(rem_len&1) 
        {
            if(mn1[i]!=a[i]) putr("NO")
        }
        else 
        {
            if((a[i]==mn1[i] and a[i+1]==mn2[i]) or (a[i]==mn2[i] and a[i+1]==mn1[i])) ;
            else putr("NO")
        }
    }
    putn("YES")
