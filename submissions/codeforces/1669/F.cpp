...
void TATAKAE(int T)
{
    ...
    partial_sum(all(w),pre.begin());
    int ans = 0;
    int last = 0;   
    for(int i=n; i>1; i--)
    {
        last += w[i];
        if(binary_search(pre.begin(),pre.begin()+i,last))
        {
            auto ind = lower_bound(pre.begin(),pre.begin()+i,last) - pre.begin();
            if(ind < i) amax(ans,ind + n-i+1);
        }
    }
    putn(ans)
}
