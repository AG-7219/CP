...
void TATAKAE(int T)
{
    get(int,n)
    get(string,s)
    vector<int> cost(n); for(int& i:cost) cin>>i;
    vector dp(5,oo);
    dp[0] = 0;
    rep(i,0,s.length())
    {
        vector<int> new_dp(5,oo);

        //dont take this character
        rep(j,0,5) amin(new_dp[j],dp[j] + cost[i]);

        //take this character
        rep(j,0,4)
        {
            amin(new_dp[j + (s[i] == "hard"[j])], dp[j]);
        }

        dp.swap(new_dp);
    }
    putn(min({dp[0],dp[1],dp[2],dp[3]}))
}
