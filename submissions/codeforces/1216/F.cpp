...
class segtree
{
    ...
    public:
        ...
        int query(int l, int r)
        {
            ...
            while(r > l)
            {
                if(l&1) amin(ans,tree[l++]);
                if(r&1) amin(ans,tree[--r]);
                l >>=1, r >>= 1;
            }
            return ans;
        }
        void update(int ind, int val)
        {
            ...
            {
                ind >>= 1;
                amin(tree[ind],min(tree[ind<<1],tree[(ind<<1) + 1]));
            }
        }
};
...
void TATAKAE(int T)
{
    ...
    {
        amin(dp[i],dp[i-1]+i);
        for(auto& [l,cost]:routers[i]) amin(dp[i],cost + root.query(l,i));
        root.update(i,dp[i]);
    }
    putn(dp.back())
}
