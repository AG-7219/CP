...
void TATAKAE(int T)
{
    ...
    partial_sum(all(twos),twos.begin());
    int ans = oo;
    if(twos.size() and twos.back() >= m) amin(ans,2*(1+lower_bound(all(twos),m) - twos.begin()));
    for(int i=0; i<ones.size(); i++)
    {
        if(ones[i] >= m) amin(ans,i+1);
        if(twos.size() and twos.back() >= m-ones[i]) amin(ans,i+1 + 2*(1+lower_bound(all(twos),m-ones[i]) - twos.begin()));
    }
    if(ans == oo) ans = -1;
    putn(ans)
}
