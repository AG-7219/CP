...
void TATAKAE(int T)
{
    get(int,n)
    get(string,s)
    vector<int> freq(26,0), o(26); for(char c:s) freq[c-'a']++;
    iota(all(o),0);
    sort_by(all(o),freq[l]>freq[r]);
    int ans = oo, arg_mn = -1;
    for(int cc=1; cc<=26; cc++)
    {
        if(n%cc == 0)
        {
            int cnt = n/cc, more = 0, less = 0;
            rep(i,0,cc) {
                if(freq[o[i]] <= cnt) less += cnt-freq[o[i]];
                else more += freq[o[i]]-cnt;
            }
            int cur = max(more,less);
            if(amin(ans,cur)) arg_mn = cc;
        }
    }
    putn(ans)
    int opt = n/arg_mn;
    ...
    rep(i,0,arg_mn)
    {
        ...
        if(freq[o[i]] < opt) 
        {
            for(int times=0; times<opt-freq[o[i]]; times++) aux.push_back(char('a'+o[i]));
        }
    }
    rep(i,0,arg_mn)
    {
