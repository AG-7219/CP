...
void TATAKAE(int T)
{
    ...
    {
        vector<int> ans;
        //we want the diff of y-x to be of form (b-a)(b+a)
        int dif = y-x;
        for(int i=1; i*i<=dif; i++)
        {
            if(dif%i == 0){
                int other = dif/i;
                if(abs(other-i)&1) continue;
                int b = (i+other)/2;
                int a = (other-i)/2;
                assert(b*b-y == a*a-x);
                if(b*b-y>=0) ans.push_back(b*b-y);
            }
        }
        return ans;
    };
    ...
    rep(i,0,n)
    {
        rep(j,0,i)
        {
            auto opt = compatible(arr[j],arr[i]);
            for(int x:opt)
            {
                if(dp[j].find(x)!=dp[j].end()) dp[i][x] = dp[j][x]+1;
                else dp[i][x] = 2;
            }
        }
    }
    int ans = 1;
    rep(i,0,n) for(auto& [x,cnt]:dp[i]) amax(ans,cnt);
    putn(ans)
}
