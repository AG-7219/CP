...
template<typename T> struct Comp1 { bool operator()(const T& l, const T& r) const { return (l.fr.fr == r.fr.fr) ? (l.sc > r.sc) : (l.fr.fr > r.fr.fr); } };
template<typename T> struct Comp2 { bool operator()(const T& l, const T& r) const { return (l.fr.sc == r.fr.sc) ? (l.sc > r.sc) : (l.fr.sc < r.fr.sc); } };
...
void solve(int T)
{
    get(int,n)
    map<pii,int> mp;
    priority_queue<pair<pii,int>,vector<pair<pii,int>>, Comp1<pair<pii,int>>> pq1;
    priority_queue<pair<pii,int>,vector<pair<pii,int>>, Comp2<pair<pii,int>>> pq2;
    while(n--)
    {
        get(int,a,b,c)
        if(mp.contains({a,b})) mp[{a,b}] = min(mp[{a,b}],c);
        else mp[{a,b}] = c;
        pq1.push({{a,b},c});
        pq2.push({{a,b},c});
        int l = pq1.top().fr.fr;
        ...
        int ans = MX;
        //try both 
        amin(ans,pq1.top().sc + pq2.top().sc);
        if(mp.contains({l,r})) amin(ans,mp[{l,r}]);
        putn(ans)
    }
}
