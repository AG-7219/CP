...
const int N = 15000001, M = 998244353, mod = 1000000007, MX = INT64_MAX, MN = INT64_MIN, oo = 1e18;
...
void solve(int T)
{
    get(int,n)
    init(v,int,n,0)
    int g = accumulate(1+all(v),0LL,[](int x, int y) { return std::gcd(x,y); });
    rep(i,1,n+1) v[i]/=g;
    vector<int> cnt(N,0);
    rep(i,1,n+1)
    {
        while(v[i] > 1)
        {
            int p = sieve[v[i]];
            v[i] /= p;
            if(sieve[v[i]]!=p) cnt[p]++;
        }
    }
    debug(cnt,0,10)
    int ans = n - *maxv(cnt,0,cnt.size());
    if(ans == n) ans = -1;
    putn(ans)
}
