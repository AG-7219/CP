...
void solve(int T)
{
    ...
    rep(i,1,n+1) rep(j,1,m+1) cin>>grid[i][j];
            ...
            if(col[i][j] == -1 and grid[i][j] == '.')
            {
                ...
                while(!st.empty())
                {
                    ...
                    rep(k,0,4)
                    {
                        int nr = r + dx[k];
                        int nc = c + dy[k];
                        if(col[nr][nc]==-1 and grid[nr][nc] == '.') 
                        {
                            st.push({nr,nc});
                            col[nr][nc] = sizes.size();
                        }
                    }
                }
                sizes.push_back(cur);
    ...
    rep(i,0,n+2) rep(j,0,m+2) if(col[i][j] == -1) col[i][j] = sizes.size();
    sizes.push_back(0);
    ...
    rep(i,1,n+1)
    {
        rep(j,1,m+1)
        {
            ...
            set<int> st;
            rep(k,0,4) st.insert(col[i+dx[k]][j+dy[k]]);
            int ans = 1;
            for(int c:st) ans += sizes[c];
            ans %=10;
            grid[i][j] = char(ans + '0');
        }
    }
