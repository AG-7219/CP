...
void TATAKAE(int T)
{
    ...
    while(hi >= lo)
    {
        ...
        rep(i,1,n+1) pre[i] = pre[i-1] + (a[i] < mid);
        rep(i,0,n+1) pre[i] *= 2;
        rep(i,0,n+1) pre[i] -= i;
        bool flag = false;
        int even = -oo, odd = -oo;
        rep(i,k,n+1)
        {
            if((i-k)&1) amax(odd,pre[i-k]);
            else amax(even,pre[i-k]);

            if(i&1)
            {
                if(pre[i] <= odd-2) flag = true;
                if(pre[i] <= even-1) flag = true;
            }
            else 
            {
                if(pre[i] <= odd-1) flag = true;
                if(pre[i] <= even-2) flag = true;
            }
        }
        if(flag)
        {
