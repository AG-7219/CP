...
{
    get(string,s,t)
    if(s.length() != t.length()) putr(-1)
    vector<vector<int>> dp(26,vector<int> (26,oo));
    rep(i,0,26) dp[i][i] = 0;
    get(int,m);
    while(m--)
    {
    ...
    rep(i,0,s.length())
    {
        int cur = oo;
        char opt = '#';
        for(char inm='a'; inm<='z'; inm++)
        {
            if(amin(cur, dp[s[i] - 'a'][inm-'a'] + dp[t[i] - 'a'][inm-'a'])) opt = inm;
        }
        if(cur == oo) putr(-1);
        s[i] = opt;
        ans += cur;
