...
void TATAKAE(int T)
{
    ...
    vector<vector<int>> dp(n, vector<int> (n+1,0)), next(n, vector<int> (n+1,-1));
    for(int i=n-1; i>=0; i--)
    {
        dp[i][1] = a[i];
        for(int len=2; len<=n-i; len++)
        {
            for(int j=i+1; j<n; j++)
            {
                if(amax(dp[i][len], dp[j][len-1])) next[i][len] = j;
                else if(dp[i][len] == dp[j][len-1] and a[j] < a[next[i][len]]) next[i][len] = j;
            }
            dp[i][len] += a[i];
        }
    }
    ...
    while(m--)
    {
        ...
        for(int i=1; i<n; i++)
        {
            if(dp[i][k] > dp[opt][k]) opt = i;
            else if(dp[i][k] == dp[opt][k] and a[i] < a[opt]) opt = i;
        }
        for(int i=0; i<pos-1; i++) opt = next[opt][k-i];
        putn(a[opt])
    }
}
