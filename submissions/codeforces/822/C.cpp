...
const int N = 200001, M = 998244353, mod = 1000000007, MX = INT64_MAX, MN = INT64_MIN, oo = 1e18;
...
{
    get(int,n,x)
    vector<tuple<int,int,int>> vouchers(n+1);
    rep(i,1,n+1)
    {
        get(int,a,b,c)
        vouchers[i] = {a,b,c};
    ...
    sort(1+all(vouchers));
    vector<int> aux(n+1);
    rep(i,1,n+1) aux[i] = get<0>(vouchers[i]);
    vector<pair<int,int>> queries[n+1];
    rep(i,1,n+1)
    {
        int ind = upper_bound(all(aux),get<1>(vouchers[i])) - aux.begin();
        if(ind<=n) queries[ind].pb(x-(get<1>(vouchers[i]) - get<0>(vouchers[i]) + 1),get<2>(vouchers[i]));
    }
    int ans = oo;
    vector<int> mn(N,oo);
    repr(i,n,0)
    {
        amin(mn[get<1>(vouchers[i]) - get<0>(vouchers[i]) + 1],get<2>(vouchers[i]));
        for(auto& [len,cost]:queries[i])
        {
            if(len>=1) amin(ans,cost + mn[len]);
        }
    }
    if(ans >= oo) ans = -1;
    putn(ans)
