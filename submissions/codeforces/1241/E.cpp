...
void TATAKAE(int T)
{
    ...
    // dp0[node] denotes the max profit in subtree of node when it is not saturated
    // dp1[node] denotes the max profit in subtree of node when it is saturated 
    ...
    function<void(int,int)> dfs = [&](int node, int par)
    {
        ...
        for(auto& [child,weight]:adj[node])
        {
            if(child!=par)
            {
                ...
                temp.push_back(weight + dp0[child] - max(dp0[child], dp1[child]));
            }
        }
        dp0[node] = dp1[node] = sum;
        sort(rall(temp));
        for(int i=0; i<min(k,(int)temp.size()); i++) 
        {
            sum += temp[i];
            if(i==k-1) amax(dp1[node],sum);
            else amax(dp0[node],sum);
        }
    };
    dfs(1,-1);
    putn(max(dp0[1],dp1[1]))
}
