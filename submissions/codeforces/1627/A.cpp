...
{
    get(int,n,m,r,c)
    vector<vector<char>> grid(n+1,vector<char> (m+1,'#')); cin>>grid;
    if(grid[r][c] == 'B') putr(0);
    int cnt = 0;
    rep(i,1,n+1) cnt += count(grid[i].begin()+1,grid[i].end(),'B');
    if(cnt == 0) putr(-1)
    if(count(grid[r].begin()+1,grid[r].end(),'B')) putr(1)
    int temp = 0;
    rep(i,1,n+1) temp += grid[i][c] == 'B';
    if(temp) putr(1)
    putn(2)
}
