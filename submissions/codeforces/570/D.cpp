...
{
    get(int,n,q)
    vector<int> adj[n+1], ans(q+1,0), qi[n+1];
    rep(i,2,n+1)
    {
        get(int,p)
        adj[i].pb(p);
    ...
    get(string,s)
    s.insert(s.begin(),' ');
    vector<pii> queries(q+1); cin>>queries;
    rep(i,1,q+1) qi[queries[i].first].push_back(i); 

    vector<map<int,bitset<26>>> sack(n+1);

    function<void(int,int,int)> dfs = [&](int node, int par, int depth)
    {
        sack[node][depth].flip(s[node]-'a');
        for(int child:adj[node])
        {
            if(child != par)
            {
                ...
                for(auto& [lvl,bs]:sack[child]) sack[node][lvl] ^= bs;
            }
        }

        for(int ind:qi[node]) ans[ind] = sack[node][queries[ind].sc].count() <= 1;
    };
