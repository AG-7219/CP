...
void TATAKAE(int T)
{
    get(int,n,a,b)
    vector<int> ans(n+1);
    for(int cnt_a=0; cnt_a*a<=n; cnt_a++)
    {
        if((n-cnt_a*a)%b) continue;
        int cnt_b = (n-cnt_a*a)/b;
        for(int i=0; i<cnt_a; i++)
        {
            vector<int> inds(a), vals(a);
            iota(all(inds),(i*a)+1);
            iota(all(vals),(i*a)+1);
            rotate(vals.begin(),vals.begin()+1,vals.end());
            for(int ind=0; ind<a; ind++) ans[inds[ind]] =vals[ind];
        }
        for(int i=0; i<cnt_b; i++)
        {
            vector<int> inds(b), vals(b);
            iota(all(inds),(a*cnt_a + i*b)+1);
            iota(all(vals),(a*cnt_a + i*b)+1);
            rotate(vals.begin(),vals.begin()+1,vals.end());
            for(int ind=0; ind<b; ind++) ans[inds[ind]] =vals[ind];
        }
        logarr(ans,1,n+1)
        return  ;
    }
