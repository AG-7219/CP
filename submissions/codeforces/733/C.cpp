...
void TATAKAE(int T)
{
    ...
    get(int,m)
    init(b,int,m,0)
    if(sumv(a,1,n+1) != sumv(b,1,m+1)) putr("NO")
    vector<pair<int,char>> ans;
    for(int i=1, j=1; i<=n and j<=m; j++)
    {
        vector<int> cur;
        for(int sum = 0, k=i; k<=n and sum!=b[j]; k++) cur.push_back(a[k]), sum += a[k];
        if(b[j] != sumv(cur,0,cur.size())) putr("NO")
        int mx = *maxv(cur,0,cur.size());
        if(cur.size() == 1) ;
        else 
        {
            if(count(all(cur), mx) == cur.size()) putr("NO");
            int ind = -1;
            for(int i=0; i<cur.size(); i++)
            {
                ...
                {
                    int nbr = 0;
                    if(i+1<cur.size() and cur[i+1]!=mx) nbr++;
                    if(i-1>=0 and cur[i-1]!=mx) nbr++;
                    if(nbr) ind = i;
                }
            }
            assert(ind!=-1);
            int flag = 0, left = ind, right = cur.size() - left - 1;
            if(ind+1<cur.size() and (cur[ind+1]==cur[ind])) flag = true;
            else { while(right--) ans.pb(j+ind,'R'); }
            while(left--) ans.pb(j+ind--,'L');
            if(flag) { while(right--) ans.pb(j+ind,'R'); }
        }
        i += cur.size();
    }
    putn("YES")
    for(auto& [ind,dir]:ans) putn(ind,dir)
}
