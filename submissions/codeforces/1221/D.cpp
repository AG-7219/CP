...
{
    get(int,n)
    vector<pii> fence(n+1); cin>>fence;
    vector<vector<int>> dp(n+1,vector<int> (3,oo));
    dp[1][0] = 0;
    dp[1][1] = fence[1].second;
    dp[1][2] = 2*fence[1].second;
    rep(i,2,n+1)
    {
        rep(cur,0,3)
        {
            rep(prev,0,3)
            {
                if(fence[i-1].first + prev != fence[i].first + cur) amin(dp[i][cur],dp[i-1][prev] + fence[i].second*cur);
            }
        }
    }
    putn(*minv(dp[n],0,3))
