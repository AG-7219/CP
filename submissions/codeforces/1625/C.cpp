...
const int N = 100001, M = 998244353, mod = 1000000007, MX = INT64_MAX, MN = INT64_MIN, oo = 1e9 + 5;
...
void solve(int T)
{
    ...
    vector<vector<int>> cur(n+1,vector<int> (k+1,oo));
    ...
    rep(i,2,n+1)
    {
        rep(j,0,k+1)
        {
            rep(last,1,i)
            {
                ...
                amin(cur[i][j], dp[last][j] + (d[i+1]-d[i])*a[i]);
                //remove this sign
                if(j<k) amin(cur[last][j+1], dp[last][j] + (d[i+1] - d[i])*a[last]);  
            }
            // {
            //     amin(cur[a[i]][j],dp[speed][j] + (d[i+1]-d[i])*a[i]);
            //     // log(dp[speed][i+1][j],dp[speed][i][j]+(d[i+1]-d[i])*a[i])

            //     if(j<k) amin(cur[speed][j+1], dp[speed][j] + (d[i+1]-d[i])*speed); 
            // }
        }
        dp.swap(cur);
        rep(j,0,k+1) rep(speed,1,dp.size()) cur[speed][j] = oo;        
    }

    int ans = oo;
    rep(speed,1,dp.size()) rep(j,0,k+1) amin(ans,dp[speed][j]);
    cout<<ans<<"\n";
}
