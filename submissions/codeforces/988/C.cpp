...
void TATAKAE(int T)
{
    ...
    rep(i,1,n+1)
    {
        get(int,k)
        int sum = 0;
        vector<int> cur(k+1); rep(j,1,k+1) cin>>cur[j], sum += cur[j];
        rep(j,1,k+1) vals.pb(sum - cur[j], i, j);
    }
    sort(all(vals));
    rep(i,1,vals.size())
    {
        if(get<0>(vals[i-1]) == get<0>(vals[i]) and (get<1> (vals[i-1]) != get<1> (vals[i])))
        {
            putn("YES")
            putn(get<1>(vals[i-1]), get<2> (vals[i-1]))
            putr(get<1>(vals[i]), get<2> (vals[i]))
        }
    }
    putn("NO")
}
