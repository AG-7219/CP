...
void TATAKAE(int T)
{
    ...
    if(odd.size() < k-p or (abs(k-p - (int)odd.size())&1) or even.size() + (odd.size() - (k-p))/2 < p) putr("NO")
    while(even.size() > p)
    {
        auto last = even.back();
        even.pop_back();
        if(p) for(int val:last) even[0].push_back(val);
        else for(int val:last) odd[0].push_back(val);
    }
    while(odd.size()>k-p and even.size()<p)
    {
        auto last = odd.back();
        odd.pop_back();
        auto slast = odd.back();
        odd.pop_back();
        even.push_back({});
        for(int val:last) even.back().push_back(val);
        for(int val:slast) even.back().push_back(val);
    }
    while(odd.size() > k-p)
    {
        auto last = odd.back();
        ...
        if(k-p)
        {
            for(int val:last) odd[0].push_back(val);
            for(int val:slast) odd[0].push_back(val);
        }
        else 
        {
            for(int val:last) even[0].push_back(val);
            for(int val:slast) even[0].push_back(val);
        }
    }
    putn("YES")
    for(auto& cur:even)
