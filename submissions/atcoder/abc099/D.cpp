...
{
    get(int,n,c)
    vector d(c+1, vector (c+1,0LL)); cin>>d;
    vector grid(n+1, vector (n+1,0LL)); cin>>grid;

    vector pre(3, vector (c+1,0LL));
    rep(i,1,n+1)
    {
        rep(j,1,n+1)
        {
            rep(color,1,c+1)
            {
                pre[(i+j)%3][color] += d[grid[i][j]][color];
            }
        }
    }
    ...
    rep(i,1,c+1)
    {
        rep(j,1,c+1)
        {
            rep(k,1,c+1)
            {
                ...
                amin(ans,pre[0][i] + pre[1][j] + pre[2][k]);
            }
        }
    }
    putn(ans)
