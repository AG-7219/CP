...
void TATAKAE(int T)
{
    ...
    sort_by(all(o),w[l] < w[r]);
    vector<int> pre(n,s[o[0]] == '1'), suf(n,s[o.back()] == '0');
    for(int i=1; i<n; i++) pre[i] = pre[i-1] + (s[o[i]] == '1');
    for(int i=n-2; i>=0; i--) suf[i] = suf[i+1] + (s[o[i]] == '0');
    
    map<int,int> ind;
    sort(all(w));
    for(int i=n-1; i>=0; i--) ind[w[i]] = i;

    int ans = min(pre.back(),suf.front());
    for(auto& [val,idx]:ind)
    {
        int cur = suf[idx];
        if(idx) cur += pre[idx-1];
        amin(ans,cur);
    }
    putn(n-ans)
}
