...
#define flush(args...)    { put(args); cout<<endl; }
#define throw(args...)    { putn(args) return ;}
...
void solve(int T)
{
    get(int,n,k)
    vector<pii> ranges(k);
    rep(i,0,k) cin>>ranges[i];
    debug(ranges,0,ranges.size())

    vector<int> dp(n+1,0), pre(n+1,0);
    //dp[i] denotes the no of ways to reach the ith cell.
    ...
    rep(i,1,n)
    {
        //calculate ans for this cell.
        pre[i] = (pre[i] + zov(pre[i-1],0,i>0) + M)%M;
        dp[i] = pre[i];

        //update ans for other people
        for(auto& [l,r]:ranges)
        {
            if(i+l<=n) pre[i+l] = (pre[i+l] + dp[i] + M)%M;
            if(i+r<n) pre[i+r+1] = (pre[i+r+1] - dp[i] + M)%M;
        }
    }
