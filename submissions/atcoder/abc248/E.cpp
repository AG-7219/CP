...
void solve(int T)
{
    //dont use c++20 stuff, gl anf hf. TATAKAE
    get(int,n,k)
    vector<pii> points(n+1); cin>>points;
    if(k == 1) putr("Infinity")

    vector<vector<pii>> lines;

    auto check = [&](pii p1, pii p2, pii cur)
    {
        int dxc = cur.first - p1.first;
        int dyc = cur.second - p1.second;

        int dxl = p2.first - p1.first;
        int dyl = p2.second - p1.second;
    ...
    rep(i,1,n+1)
    {
        rep(j,i+1,n+1)
        {
            ...
            rep(point,1,n+1)
            {
                if(check(points[i],points[j],points[point])) cur.pb(points[point]);
            }
            sort(all(cur));
            lines.pb(cur);
        }
    ...
    uniq(lines);
    int ans = 0;
    for(auto& line:lines) ans += line.size() >= k;
    putn(ans)
}
