...
void TATAKAE(int T)
{
    get(int,n)
    init(a,int,n,0)
    int arg_mn = minv(a,1,n+1)-a.begin(), arg_mx = maxv(a,1,n+1)-a.begin();
    vector<pii> ans;
    if(a[arg_mn]<0 and a[arg_mx]>0){
        if(abs(a[arg_mn]) > abs(a[arg_mx])){
            rep(i,1,n+1) if(a[i] > 0) ans.pb(arg_mn,i), a[i] += a[arg_mn];
        }
        else{
            rep(i,1,n+1) if(a[i] < 0) ans.pb(arg_mx,i), a[i] += a[arg_mx];
        }
    }
    if(a[arg_mn]<0){
        repr(i,n-1,0) if(a[i]>a[i+1]) ans.pb(i+1,i), a[i] += a[i+1];
    }
    else{
        rep(i,2,n+1) if(a[i-1] > a[i]) ans.pb(i-1,i), a[i] += a[i-1];
    }
    debug(a)
    putn(ans.size())
    for(auto& [x,y]:ans) putn(x,y)
}
