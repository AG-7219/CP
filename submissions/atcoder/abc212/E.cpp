...
void TATAKAE(int T)
{
    ...
    for(int i=0; i<=k; i++)
    {
        ...
        {
            dp[i][node] = (dp[i][node] + prev + M)%M;
            next = (next + dp[i][node])%M;
            if(i+1<=k)
            {
                for(int nbr:adj[node]) dp[i+1][nbr] = (dp[i+1][nbr] - dp[i][node] + M)%M;
                dp[i+1][node] = (dp[i+1][node] - dp[i][node] + M)%M;
            }
        }
        prev = next;
    }
