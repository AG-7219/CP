...
// dp[node] = (n-1)! * pi(in[childs])/ pi(sub[childs]!)
// dp[child] = (n-1)! * pi(in[childs]) / pi(sub[childs]!)

//dp[child] = (n-1)! * pi(in[subtree childs]) * dp[parent subtree without me] / pi(sub[subtree childs]!) * (n - sub[child])!

//dp[child] = (n-1)! * in[child] * dp[parent subtree without me] / (n - sub[child])! * sub[child]! 
...
void TATAKAE(int T)
{
    ...
    vector<int> facts(n+1,1), inv(n+1);
    rep(i,1,n+1) facts[i] = (facts[i-1] * i)%mod;
    rep(i,0,n+1) inv[i] = expm(facts[i],mod-2);

    vector<int> in(n+1,1), dp(n+1,1), sub(n+1,0);
    ...
    function<void(int,int)> dfs1 = [&](int node, int par)
    {
        for(int child:adj[node])
        {
            ...
            {
                dfs1(child,node);
                in[node] = (in[node] * inv[sub[child]])%mod;
                in[node] = (in[node] * in[child])%mod;
                sub[node] += sub[child];
            }
        }
        in[node] = (in[node] * facts[sub[node]++])%mod;
    };
    ...
    function<void(int,int)> dfs2 = [&](int node, int par)
    {
        for(int child:adj[node])
        {
            ...
            {
                //removing this child
                dp[node] = (dp[node] * expm(in[child],mod-2))%mod;
                dp[node] = (dp[node] * facts[sub[child]])%mod;
                dp[node] = (dp[node] * inv[n-1])%mod;
                dp[node] = (dp[node] * facts[n-1-sub[child]])%mod;

                debug(node,child,dp[node])
                //setting dp[child]
                //we have two subtrees to merge now
                dp[child] = (in[child] * dp[node])%mod;
                dp[child] = (dp[child] * facts[n-1])%mod;
                dp[child] = (dp[child] * inv[sub[child]-1])%mod;
                dp[child] = (dp[child] * inv[n - sub[child]])%mod;

                dfs2(child,node);

                //reverting changes
                dp[node] = (dp[node] * inv[sub[child]])%mod;
                dp[node] = (dp[node] * in[child])%mod;
                dp[node] = (dp[node] * facts[n-1])%mod;
                dp[node] = (dp[node] * inv[n-1-sub[child]])%mod; 
            }
        }
    };
