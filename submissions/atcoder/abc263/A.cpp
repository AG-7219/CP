...
void TATAKAE(int T)
{
    vector<int> arr(5); for(int& i:arr) cin>>i;
    sort(all(arr));
    if(arr[0]==arr[1] and arr[1]==arr[2] and arr[3]==arr[4] and arr[2] != arr[3])putr("Yes")
    if(arr[0]==arr[1] and arr[1]!=arr[2] and arr[3]==arr[2] and arr[4] == arr[3])putr("Yes")
    putn("No")
}
