...
void solve(int T)
{
    get(int,n,m)
    map<int,int> mp;
    rep(i,2,(int)sqrt(m)+1)
    {
        while(m%i == 0) mp[i]++, m/=i;
    }
    if(m > 1) mp[m]++;
    int ans = 1;
    vector<int> facts(N+100,1), inv(N+100);
    rep(i,2,facts.size()) facts[i] = (facts[i-1] *i)%mod;
    rep(i,0,inv.size()) inv[i] = expm(facts[i],mod-2,mod);

    auto nCr = [&](int p, int q)
    {
        assert(p>=q);
        return (((facts[p]*inv[q])%mod)*inv[p-q])%mod;
    };

    for(auto& [prime,exp]:mp)
    {
        ans = (ans * nCr(n-1+exp,exp))%mod;
    }
    putn(ans)
}
