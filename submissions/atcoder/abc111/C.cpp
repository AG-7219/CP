...
void solve(int T)
{
    get(int,n)
    init(v,n)
    vector<int> hsh1(N,0), hsh2(N,0);
    rep(i,0,n)
    {
        if(i&1) hsh2[v[i]]++;
        else hsh1[v[i]]++;
    }
    int mx1 = maxv(hsh1,0,N)-hsh1.begin();
    int mx2 = maxv(hsh2,0,N)-hsh2.begin();
    sort(all(hsh1),greater<ll>());
    sort(all(hsh2),greater<ll>());
    ll ans = INT_MAX;
    if(mx1 == mx2)
    {
        ans = min(n - hsh1[0] - hsh2[1], n - hsh1[1] - hsh2[0]);
    }
    else ans = n - hsh1[0] - hsh2[0];
    putn(ans)
}
