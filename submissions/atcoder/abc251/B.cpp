...
void TATAKAE(int T)
{
    ...
    rep(i,0,n)
    {
        if(weights[i] <= w) ans[weights[i]] = true;
        rep(j,i+1,n)
        {
            if(weights[i] + weights[j]<= w) ans[weights[i] + weights[j]] = true;
            rep(k,j+1,n) 
            {
                if(weights[i] + weights[j] + weights[k] <= w) ans[weights[i] + weights[j] + weights[k]] = true;
            }
        }
    }
    putn(sumv(ans,0,w+1))
