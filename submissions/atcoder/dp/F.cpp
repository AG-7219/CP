...
void solve(int T)
{
    ...
    vector<vector<pii>> back(n,vector<pii> (m,{n,m}));
    repr(i,n-1,-1)
    {
        repr(j,m-1,-1)
        {
            if(s[i] == t[j])
            {
                dp[i][j] = 1 + zov(dp[i+1][j+1],0,(i<n-1 and j<m-1));
                back[i][j] = {i+1,j+1};
            }
            else
            {
                if(amax(dp[i][j],zov(dp[i+1][j],0,i+1<n))) back[i][j] = {i+1,j};
                if(amax(dp[i][j],zov(dp[i][j+1],0,j+1<m))) back[i][j] = {i,j+1};
            }
        }
    }
    if(dp[0][0] == 0) putr()
