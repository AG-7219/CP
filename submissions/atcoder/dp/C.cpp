...
int main(int argc, char const *argv[])
{
    ...
    while(t--)
    {
        ...
        rep(j,0,2) dp[j][1] = tasks[j][1];
        rep(i,2,n)
        {
            dp[0][i] = tasks[0][i] + max(dp[1][i-1], dp[2][i-1]);
            dp[1][i] = tasks[1][i] + max(dp[0][i-1], dp[2][i-1]);
            dp[2][i] = tasks[2][i] + max(dp[1][i-1], dp[0][i-1]);
        }
        putn(max({dp[0][n],dp[1][n],dp[2][n]}))
    }
