    ...
    function<void(int,int)> dfs = [&](int node, int par)
    {
        ...
        for(int child:adj[node])
        {
            if(child!=par)
            {
                leaf = false;
                dfs(child,node);
                amin(ans[node].first,ans[child].first);
                amax(ans[node].second,ans[child].second);
            }
        }
        if(leaf)
        {
            ans[node] = {timer,timer};
            timer++;
        }
        assert(ans[node].first!=-1);
        assert(ans[node].second!=-1);
    };
    dfs(1,-1);
