...
void solve(int T)
{
    ...
    get(int,n,m)
    vector<pii> chocolates(n), boxes(m);
    ...
    sort(all(boxes));
    vector<int> aux;
    for(auto& [x,y]:chocolates) aux.push_back(x);
    ...
    for(auto& [x,y]:boxes)
    {
        ...
        if(it!=aux.begin())
        {
            queries[it-aux.begin()-1].push_back(y);
        }
    }
    debug(queries,chocolates,boxes)
    oset(pii) os;
    rep(i,0,n)
    {
        os.insert({chocolates[i].second,i});
        for(int y:queries[i])
        {
            if(os.empty()) continue;
            auto it = os.upper_bound({y,oo});
            if(it!=os.begin()) os.erase((--it));
            debug(y,os)
        }
    }
    if(os.empty()) putn("Yes")
    else putn("No")
}
