...
void solve(int T)
{
    ...
    init(a,int,n,0)
    init(b,int,n,0)
    vector dp(n+1,vector (2,false));
    dp[1][0] = dp[1][1] = true;
    rep(i,2,n+1)
    {
        if(abs(a[i] - a[i-1])<=k and dp[i-1][0]) dp[i][0] = true;
        if(abs(a[i] - b[i-1])<=k and dp[i-1][1]) dp[i][0] = true; 
        if(abs(b[i] - a[i-1])<=k and dp[i-1][0]) dp[i][1] = true;
        if(abs(b[i] - b[i-1])<=k and dp[i-1][1]) dp[i][1] = true; 
    }
    if(dp[n][0] or dp[n][1]) putr("Yes")
    putn("No")
}
