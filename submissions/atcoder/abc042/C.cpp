...
#define get(args...)    long long args; input(args);
#define put(args...)    output(args); cout<<"\n";
...
void input() {}
template<typename T, typename... Args>
void input(T& a, Args&... args) { cin>>a; input(args...); }

void output() {}
template<typename T, typename... Args>
void output(T&& a, Args&&... args) { cout<<a<<" "; output(args...); }
...
void f(ll& n, vector<bool>& hsh, int asf, int level = 1)
{
    if(asf >= n)
    {
        ans = min(ans,asf);
    ...
    for(int i=0; i<10; i++)
    {
        if(hsh[i]) f(n,hsh,asf*10+i,level+1);
    }
}
