...
void solve(int T)
{
    ...
    vector<vector<int> > row1(h+2,vector<int> (w+2,0)), row2(h+2,vector<int> (w+2,w+1));
    vector<vector<int> > col1(h+2,vector<int> (w+2,0)), col2(h+2,vector<int> (w+2,h+1));

    rep(i,1,h+1) rep(j,1,w+1) row1[i][j] = (mat[i][j] == '#') ? j : row1[i][j-1];
    rep(i,1,h+1) repr(j,w,0) row2[i][j] = (mat[i][j] == '#') ? j : row2[i][j+1];
    rep(i,1,w+1) rep(j,1,h+1) col1[j][i] = (mat[j][i] == '#') ? j : col1[j-1][i];
    rep(i,1,w+1) repr(j,h,0) col2[j][i] = (mat[j][i] == '#') ? j : col2[j+1][i];
    ...
    rep(i,1,h+1)
    {
        rep(j,1,w+1)
        {
            int cur = row2[i][j] - row1[i][j] - 1;
            cur += col2[i][j] - col1[i][j] - 1;
            ans = max(ans,cur-1);
        }
    }
    putn(ans)
