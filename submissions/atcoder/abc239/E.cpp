...
void solve(int T)
{
    ...
    vector<int> inds[n+1];
    rep(i,1,n+1) inds[queries[i].fr].push_back(i);
    vector<int> ans(q+1);
    vector<map<int,int,greater<>>> sack(n+1);
    ...
    function<void(int,int)> dfs = [&](int node,int par)
    {
        ...
        for(int child:adj[node])
        {
            ...
            {
                dfs(child,node);
                if(sack[child].size() > sack[node].size()) swap(sack[child],sack[node]);
                for(auto& [val,cnt]:sack[child]) sack[node][val] += cnt;
            }
        }
        for(int qi:inds[node])
        {
            int cur = queries[qi].second;
            for(auto& [val,cnt]:sack[node])
            {
                if(cnt >= cur)
                {
                    ans[qi] = val;
