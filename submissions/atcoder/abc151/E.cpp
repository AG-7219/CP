...
void solve(int T)
{
    ...
    sort(all(arr));
    vector<int> facts(n+1,1), inv(n+1,0);
    rep(i,1,facts.size()) facts[i] = (facts[i-1] * i)%mod;
    rep(i,0,inv.size()) inv[i] = expm(facts[i],mod-2);
    ...
    int ans = 0;
    for(int i=0; i<n; i++) ans = (ans + (nCr(i,k-1)*arr[i])%mod)%mod;
    for(int i=0; i<n; i++) ans = (ans - (nCr(n-i-1,k-1)*arr[i])%mod + mod)%mod;
    putn(ans)
}
