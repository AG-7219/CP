...
void put() {}
template<typename T, typename... Args>
void put(T&& a, Args&&... args) { cout<<a<<" "; put(args...); }
...
bool check(string& s, int ind, string& t)
{
    int i = 0, j =ind;
    while(i<t.size() and j<s.size())
    {
        if((s[j] != t[i++]) and s[j]!='?') return false;
        j++;
    }
    return (j-ind == t.length());
}

void fill(string& s, int ind, string& t)
{
    for(int i=ind, j=0; j<t.size(); i++, j++) s[i] = t[j];
...
void solve(int T)
{
    ...
    repr(i,n-1,-1)
    {
        if((s[i] == '?' or s[i] == t[0]) and check(s,i,t)) 
        {
            fill(s,i,t); 
            putn(s)
            return ;
