...
void TATAKAE(int T)
{
    ...
    vector<int> in(n+1,-oo), out(n+1,-oo);
    function<void(int,int)> dfs1 = [&](int node, int par)
    {
        for(auto& [child,w]:adj[node])
        {
            ...
            dfs1(child,node);
            amax(in[node],w + d[child]);
            amax(in[node],w + in[child]);
        }
    };
    ...
    function<void(int,int)> dfs2 = [&](int node, int par)
    {
        ...
        {
            if(child!=par) pre.pb(wt+max(in[child],d[child])), suf.pb(wt+max(in[child],d[child]));
            else pre.pb(-oo), suf.pb(-oo);
        }
        partial_sum(all(pre),pre.begin(),g(l,r,max(l,r)));
        partial_sum(rall(suf),suf.rbegin(),g(l,r,max(l,r)));
        debug(node,pre,suf)
        for(int i=0; i<adj[node].size(); i++)
        {
            auto [child,wt] = adj[node][i];
            if(child == par) continue;

            amax(out[child], wt + max(out[node],d[node]));
            if(i) amax(out[child], wt+pre[i-1]);
            if(i+1 < adj[node].size()) amax(out[child], wt+suf[i+1]);
            dfs2(child,node);
        }
    };
    ...
    dfs2(1,-1);
    debug(in,out)
    rep(i,1,n+1) putn(max(in[i],out[i]))
}
