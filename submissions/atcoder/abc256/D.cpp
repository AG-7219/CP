...
{
    get(int,n) 
    vector<pii> intervals(n+1); cin>>intervals;
    sort_by(1+all(intervals), r.fr > l.fr);
    stack<pii> st;
    rep(i,1,n+1)
    {
        if(st.empty()) st.push(intervals[i]);
        else if(st.top().second >= intervals[i].first) amax(st.top().second, intervals[i].second);
        else st.push(intervals[i]);
    }
    vector<pii> ans;
    while(st.size()) ans.pb(st.top()), st.pop();
    reverse(all(ans));
