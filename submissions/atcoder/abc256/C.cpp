...
void TATAKAE(int T)
{
    get(int,h1,h2,h3,w1,w2,w3)
    
    int grid[3][3];

    auto fill_and_check = [&]()
    {
        grid[0][2] = h1-grid[0][0]-grid[0][1];
        grid[1][2] = h2-grid[1][0]-grid[1][1];
        grid[2][0] = w1-grid[0][0]-grid[1][0];
        grid[2][1] = w2-grid[0][1]-grid[1][1];
        grid[2][2] = w3-grid[0][2]-grid[1][2];

        rep(i,0,3) rep(j,0,3) if(grid[i][j] <= 0) return false;
        return grid[2][0] + grid[2][1] + grid[2][2] == h3;
    };
    ...
    rep(i,1,31)
    {
        rep(j,1,31)
        {
            rep(k,1,31)
            {
                rep(l,1,31)
                {
                    grid[0][0] = i, grid[0][1] = j;
                    grid[1][0] = k, grid[1][1] = l;
                    ans += fill_and_check();
                }
            }
        }
