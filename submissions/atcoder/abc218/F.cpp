...
{
    get(int,n,m)
    vector<pii> edges(m+1), adj[n+1]; 
    vector<bool> used(m+1,false);
    rep(i,1,m+1)
    {
        cin>>edges[i];
        adj[edges[i].first].pb(edges[i].sc,i);
    }

    auto bfs = [&]()
    {
        queue<int> q;
        vector<int> dist(n+1,oo), last(n+1,-1);
        q.push(1);
        dist[1] = 0;
        while(!q.empty())
        {
            ...
            for(auto& [child,ind]:adj[node]) if(amin(dist[child],dist[node]+1)) q.push(child), last[child] = ind;
        }
        int node = n;
        while(node!=1 and last[node]!=-1)
        {
            used[last[node]] = true;
            node = edges[last[node]].fr;
        }
        return dist[n];
    };
    int res = bfs();
    vector<int> ans(m+1,-1);
    rep(i,1,m+1) if(!used[i]) ans[i] = res;
    rep(i,1,m+1)
    {
        if(ans[i] == -1)
        {
            auto it = find(all(adj[edges[i].first]),make_pair(edges[i].second,i));
            adj[edges[i].first].erase(it);
            ans[i] = bfs();
