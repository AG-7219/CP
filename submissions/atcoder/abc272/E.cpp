...
void TATAKAE(int T)
{
    ...
    rep(i,0,n)
    {
        ...
        {
            int mid = mid(lo,hi);
            if(a[i] + (i+1)*mid >= 0) ans = mid, hi = mid-1;
            else lo = mid+1;
        }
        if(ans!=-1) add[ans].push_back(i);
        
        lo = 1, hi = m, ans = -1;
        while (hi >= lo)
        {
            int mid = mid(lo,hi);
            if(a[i] + (i+1)*mid > n) ans = mid, hi = mid-1;
            else lo = mid+1;
        }
        if(ans!=-1) remove[ans].push_back(i);
    }
    ...
    auto get_mex = [&](int round)
    {
        vector<int> cur;
        for(int ind:relevant) cur.push_back(a[ind] + (ind+1)*round);
        sort(all(cur));
        uniq(cur);
        for(int i=0; i<cur.size(); i++) if(cur[i]!=i) return i;
        return (int)cur.size();
    };

    rep(i,1,m+1)
    {
        for(int ind:remove[i]) relevant.erase(ind);
        for(int ind:add[i]) relevant.insert(ind);
        ans.pb(get_mex(i));
    }
