...
{
    get(string,s)
    if(s.front() >= 'A' and s.front() <= 'Z') s.erase(s.begin());
    else putr("No")
    if(s.empty()) putr("No")
    if(s.back() >= 'A' and s.back() <= 'Z') s.pop_back();
    else putr("No")
    if(s.empty() or s.length()!=6) putr("No")
    for(char c:s) if(c<'0' or c>'9') putr("No")
    if(stoi(s) >= 100000 and stoi(s) <= 999999) putr("Yes")
    putn("No")
}
