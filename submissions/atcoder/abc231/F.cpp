...
void solve(int T)
{
    get(int,n)
    vector<pii> pairs(n);
    for(int i=0; i<n; i++) cin>>pairs[i].fr;
    for(int i=0; i<n; i++) cin>>pairs[i].sc;
    sort_by(all(pairs),((l.fr == r.fr) ? (l.sc > r.sc) : (l.fr < r.fr)));
    int ans = 0;
    oset(pii) os;
    for(int i=0; i<n; i++)
    {
        int cnt = 0;
        int j = i;
        while(j<n and pairs[i]==pairs[j]) 
        {
            cnt++;
            os.insert({pairs[j].sc,j});
            j++;
        }
        ans += (os.size() - os.order_of_key({pairs[i].sc,-1}))*cnt;
        i = j-1;
    }   
    putn(ans)
}
