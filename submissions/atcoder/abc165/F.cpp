...
void TATAKAE(int T)
{
    ...
    vector<int> aux(1+all(val));
    sort(all(aux));
    uniq(aux);

    segtree root(vector<int> (aux.size(),0LL), 0LL, [](int val, int ind) { return val; }, [](int l, int r) { return max(l,r); }, 0LL, [](int up, int node) { return up; });

    vector<int> ans(n+1);
    function<void(int,int)> dfs = [&](int node, int par)
    {
        int ind = lower_bound(all(aux),val[node]) - aux.begin();
        int temp = root.query(ind);
        if(root.query(0,ind)+1 > temp) root.update(ind,root.query(0,ind)+1);
        ans[node] = root.query(0,aux.size());
