...
void TATAKAE(int T)
{
    ...
    init(c,int,n,0)
    init(x,int,n,0)
    oset(pii) os, buckets[n+1];
    int ans = 0;
    rep(i,1,n+1)
    {
        //add all elements to the left strictly greater than you.
        ans += os.size() - os.order_of_key({x[i],i});
        
        //remove all elements with the same color
        ans -= buckets[c[i]].size() - buckets[c[i]].order_of_key({x[i],i});

        os.insert({x[i],i});
        buckets[c[i]].insert({x[i],i});
    }
    putn(ans)
}
