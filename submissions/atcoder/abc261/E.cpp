...
{
    get(int,n,c)
    vector<int> state(30,2);

    auto setb = [&](int bit) { c |= 1<<bit; };
    auto unset = [&](int bit) { int mask = (1<<30)-1; mask ^= 1<<bit; c&=mask; };
    auto flip = [&](int bit) { c ^= 1<<bit; };
    ...
    while(n--)
    {
        ...
        {
            if(state[bit] == 0) unset(bit);
            else if(state[bit] == 1) setb(bit);
            else if(state[bit] == 3) flip(bit);
        }
        if(t==1)
        {
            c &= a;
        ...
        {
            c |= a;
            for(int bit=0; bit<30; bit++) if((a>>bit)&1) state[bit] = 1;
        }
        else 
        {
            c ^= a;
            for(int bit=0; bit<30; bit++) state[bit] ^= (a>>bit)&1;
        }
        putn(c)
    }
}
