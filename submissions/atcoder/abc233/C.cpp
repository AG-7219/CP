...
{
    get(int,n,x)
    vector<vector<int>> items(n+1,vector<int> ());
    rep(i,1,n+1)
    {
        get(int,k)
        items[i].resize(k+1);
    ...
    vector<map<int,int>> ways(n+1);
    rep(i,1,items.back().size()) ways[n][items.back()[i]] += 1;

    repr(i,n-1,0)
    {
        for(int j=1; j<items[i].size(); j++)
        {
            for(auto [val2,way]:ways[i+1])
            {
                if(MX/val2 < items[i][j]) continue;
                ways[i][items[i][j]*val2] += way;
            }
        }
    }
