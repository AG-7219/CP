...
void TATAKAE(int T)
{
    ...
    rep(i,2,n+1)
    {
        vector<int> pre(m+1,0), suf(m+1,dp.back());
        rep(j,1,m+1) pre[j] = (pre[j-1] + dp[j])%M;
        repr(j,m-1,0) suf[j] = (suf[j+1] + dp[j])%M;

        fill(all(dp),0);
        rep(j,1,m+1)
        {
            if(k+j+(k==0) < suf.size()) dp[j] = suf[k+j+(k==0)];
            if(j-k >= 1) dp[j] = (dp[j] + pre[j-k])%M;
        }
    }
    int ans = 0;
    rep(i,1,m+1) ans = (ans + dp[i])%M;
    putn(ans)
}
