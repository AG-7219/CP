...
{
    get(int,h,w)
    vector<vector<int>> grid(h, vector<int> (w,0)), dp(h, vector<int> (w,-oo));
    dp[h-1][w-1] = 0;
    rep(i,0,h)
    {
        rep(j,0,w)
    ...
    for(int row=h-1; row>=0; row--)
    {
        ...
        {
            if(row+1<h) amax(dp[row][col],grid[row+1][col] - dp[row+1][col]);
            if(col+1<w) amax(dp[row][col],grid[row][col+1] - dp[row][col+1]);
        }
    }
    debug(dp)
    if(dp[0][0] > 0) putr("Takahashi")
    else if(dp[0][0] < 0) putr("Aoki")
    putr("Draw")
}
