...
{
    get(int,h,w,n)
    vector<array<int,3>> points(n); for(auto& [val,r,c]:points) cin>>r>>c>>val;
    vector<int> o(n), dp(n,0); iota(all(o),0); sort_by(all(o),points[l][0] > points[r][0]);
    vector<int> row(h+1,0), col(w+1,0);
    for(int i=0; i<o.size(); )
    {
        int j;
        for(j=i; j<o.size() and points[o[j]][0]==points[o[i]][0]; j++)
        {
            auto [val,r,c] = points[o[j]];
            dp[o[j]] = max(row[r],col[c]);
        }
        for(j=i; j<o.size() and points[o[j]][0]==points[o[i]][0]; j++)
        {
            auto [val,r,c] = points[o[j]];
