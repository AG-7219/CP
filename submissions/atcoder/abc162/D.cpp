...
void solve(int T)
{
    ...
    rep(i,2,n)
    {
        ...
        if(ind == 0)
        {
            ans += dp[1][i-1]*(dp[2][n] - dp[2][i]);
            ans += dp[2][i-1]*(dp[1][n] - dp[1][i]);
        }
        else if(ind == 1)
        {
            ans += dp[2][i-1]*(dp[0][n] - dp[0][i]);
            ans += dp[0][i-1]*(dp[2][n] - dp[2][i]);
        }
        else 
        {
            ans += dp[0][i-1]*(dp[1][n] - dp[1][i]);
            ans += dp[1][i-1]*(dp[0][n] - dp[0][i]);
        }
    }
    rep(i,1,n+1)
    {
        rep(j,i+1,n+1)
        {
            if((i+j)%2) continue;
            int mid = (i+j)/2;
            if(s[i]!=s[j] and s[mid]!=s[j] and s[mid]!=s[i]) ans--;
        }
    }
    putn(ans)
}
