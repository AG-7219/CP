...
void solve(int T)
{
    ...
    rep(i,0,n)
    {
        vector<char> arr(all(grid[i]));
        if(helper(arr)) putr("Yes")
    }

    rep(i,0,n)
    {
        vector<char> arr;
        rep(j,0,n) arr.push_back(grid[j][i]);
        if(helper(arr)) putr("Yes")
    }
    ...
    while(row<grid.size())
    {
        ...
        while(rowTemp>=0)
        {
            arr.push_back(grid[rowTemp--][col++]);
        }
        if(helper(arr)) putr("Yes")
        row++;
    }
    ...
    while(col<grid.size())
    {
        ...
        int colTemp = col;
        row = grid.size()-1;
        while(colTemp<=grid.size()-1)
        {
            arr.push_back(grid[row--][colTemp++]);
        }
        if(helper(arr)) putr("Yes")
        col++;
    }
    ...
    repr(dif,n-1,-1)
    {
        ...
        {
            int j = dif + i;
            if(j>=0 and j<n) arr.push_back(grid[i][j]);
            else break;
        }
        debug(arr)
        if(helper(arr)) putr("Yes")
