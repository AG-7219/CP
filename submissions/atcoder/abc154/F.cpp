...
const int N = 5000001, M = 998244353, mod = 1000000007, MX = INT64_MAX, MN = INT64_MIN, oo = 1e18;
...
void TATAKAE(int T)
{
    ...
    rep(i,1,N) facts[i] = (facts[i-1] * i)%mod;
    rep(i,0,N) inv[i] = expm(facts[i],mod-2);

    auto nCr = [&](int n, int r)
    {
        if(r>n) return 0LL;
        return (((facts[n]*inv[r])%mod) * inv[n-r])%mod;
    };
    ...
    rep(i,r1,r2+1)
    {
        ans = (ans + nCr(i+c2+1,c2))%mod;
        ans = (ans - nCr(i+c1,c1-1) + mod)%mod;
    }
    putn(ans)
}
