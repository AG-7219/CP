...
{
    get(int,n,a,b,c)
    vector<int> l(n); for(int& sticks:l) cin>>sticks;

    auto helper = [&](int submask, int len)
    {
        int sum = 0, cost = max(0,(bpc(submask)-1))*10;
        for(int i=0; i<n; i++) sum += ((submask>>i)&1)*l[i];
        cost += abs(sum - len);
        return cost;
    };  
    ...
    for(int mask1=0; mask1<(1<<n); mask1++)
    {
        for(int mask2=0; mask2<(1<<n); mask2++)
        {
            ...
            int conjugate = (1<<n)-1;
            conjugate ^= (mask1 | mask2);
            amin(ans,helper(mask1,a) + helper(mask2,b) + dp[conjugate]);
        }
    }
    cout<<ans<<"\n";
}
