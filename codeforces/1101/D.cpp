...
void solve(int T)
{
	...
	{
		map<int,vector<int>> spanning;
		for(int prime: primes[a[node]]) dp[node][prime] = 1, amax(ans,dp[node][prime]);
		for(int child:adj[node])
		{
			if(child!=par)
			{
				dfs(child,node);
				for(int prime:primes[a[node]])
				{
					amax(dp[node][prime],dp[child][prime]+1);
					amax(ans,dp[node][prime]);
					spanning[prime].push_back(dp[child][prime]);
				}
			}
		}
		for(auto& [prime,paths]:spanning)
		{
			sort(all(paths));
			if(paths.size() >= 2) amax(ans,paths.back()+paths[paths.size()-2]+1);
		}
	};
