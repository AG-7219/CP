...
void TATAKAE(int T)
{
    get(int,h)
    int n = 0;
    vector<int> a(h+1,0); for(int& val:a) cin>>val, n += val;
    vector<int> ans1(n,0), ans2(n,0);
    vector<int> prev;
    prev.push_back(1);
    for(int i=1, sum = 1; i<=h; i++)
    {
        ans2[sum] = prev.front();
        for(int cur=sum; cur<sum+a[i]; cur++)
        {
            ans1[cur] = prev.front();
            if(cur>sum) ans2[cur] = prev.back();
        }
        prev.clear();
        for(int cur=sum+1; cur<=sum+a[i]; cur++) prev.push_back(cur);
        sum += a[i];
    }
    if(ans1 == ans2) putr("perfect")
    putn("ambiguous")
    logarr(ans1,0,n)
    logarr(ans2,0,n)
}
