...
class segtree
{
    ...
    public:
        ...
        segtree(vector<int>& arr)
        {
            ...
            for(int i=0; i<arr.size(); i++) tree[i+arr.size()] = arr[i];
            for(int i=arr.size()-1; i>0; i--) tree[i] = tree[2*i] + tree[2*i+1];
        }
...
void TATAKAE(int T)
{
    ...
    int cnt = 0;
    rep(i,0,n) rep(j,0,m) cin>>grid[i][j], cnt += grid[i][j] == '*';
    vector<segtree> helper(m);
    rep(i,0,m)
    {
        vector<int> cur(n,0);
        rep(j,0,n) cur[j] = (grid[j][i] == '.');
        helper[i] = segtree(cur);
    }
    vector<int> cur(m,0);
    rep(i,0,m) rep(j,0,n) cur[i] += grid[j][i] == '.';
    segtree root(cur);
    while(q--)
    {
        get(int,x,y)
        ...
        int full = cnt/n;
        int rem = cnt - full*n;
        int ans = root.query(0,full);
        ans += helper[full].query(0,rem);
        putn(ans)
    }
}
