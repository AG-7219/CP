...
void solve(int T)
{
    ...
    rep(i,1,n+1) inds[v[i]].push_back(i);
    
    for(auto& [val,pos]:inds)
    {
        sort(all(pos));
        for(int i=1; i<pos.size(); i+=2)
        {
            int fst = pos[i-1];
            int scn = pos[i];
            inds[2*val].push_back(scn);
        }
        if(pos.size()%2 == 0) pos.clear();
        else 
        {
            int temp = pos.back();
            pos.clear();
    ...
    for(auto& [val,pos]:inds)
    {
        if(pos.size()) pairs.push_back({pos.front(),val});
    }
    sort(all(pairs));
    cout<<pairs.size()<<"\n";
    for(auto p:pairs) cout<<p.sc<<" ";
