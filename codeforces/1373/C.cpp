...
void TATAKAE(int T)
{
    ...
    rep(i,1,s.length()) pre[i] = pre[i-1] + ((s[i] == '+') ? 1 : -1);
    rep(i,1,s.length()) amin(pre[i], pre[i-1]);
    int mn = max(0LL,-pre.back());
    int ans = 0;
    rep(i,0,mn+1)
    {
        int lo = 0, hi = pre.size()-1, res = -1;
        while(hi >= lo)
        {
            int mid = mid(lo,hi);
            if(-pre[mid] > i) res = mid, hi = mid-1;
            else lo = mid+1;
        }
        if(res == -1) res = s.length()-1;
        ans += res+1;
    }
    putn(ans)
}
