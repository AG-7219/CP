...
{
    get(int,n)
    vector<int> a(n), ans(n); for(int& val:a) cin>>val;
    sort(rall(a));
    for(int i=1; i<n; i+=2) ans[i] = a.back(), a.pop_back();
    for(int i=0; i<n; i+=2) ans[i] = a.back(), a.pop_back();
    int res = 0;
    for(int i=1; i<n; i+=2)
    {
        if(i+1<n and (ans[i-1]>ans[i]) and (ans[i]<ans[i+1])) res++;
    }
    putn(res)
    logarr(ans,0,n)
}
