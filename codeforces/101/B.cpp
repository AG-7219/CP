...
{
    get(int,n,m)
    vector<pii> buses(m+1); cin>>buses;
    vector<int> aux;
    rep(i,1,m+1) aux.push_back(buses[i].fr), aux.push_back(buses[i].sc);
    sort(all(aux));
    uniq(aux)
    if(aux.empty() or aux.front() or aux.back()!=n) putr(0)
    vector<int> dp(aux.size(),0), pre(aux.size(),0), tele[aux.size()];
    rep(i,1,m+1) tele[lower_bound(all(aux),buses[i].sc) - aux.begin()].push_back(lower_bound(all(aux),buses[i].fr)-aux.begin());
    dp[0] = 1;
    pre[0] = 1;
    rep(i,1,aux.size())
    {
        for(int s:tele[i])
        {
            dp[i] = (dp[i] + pre[i-1] - zov(pre[s-1],0,s) + mod)%mod;
        }
