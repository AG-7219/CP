...
void TATAKAE(int T)
{
    ...
    if(n==1 and m==1) putr(grid[0][0] == k);
    vector<vector<map<int,int>>> ways(n, vector<map<int,int>> (m, map<int,int> ()));
    int ops = (n-1 + m-1);
    int left = ops/2, right = ops - left-1;
    for(int mask=0; mask<(1<<left); mask++)
    {
        int r = 0, c = 0, cur=grid[0][0], flag = 1;
        for(int i=0; i<left; i++)
        {
            if((mask>>i)&1) r++;
            else c++;
            if(r>=n or c>=m) { flag = 0; break; }
            cur ^= grid[r][c];
        }
        if(!flag) continue;
        debug(r,c,cur)
    ...
    {
        int r = n-1, c = m-1, cur = grid[n-1][m-1], flag = 1;
        for(int i=0; i<right; i++)
        {
            if((mask>>i)&1) r--;
            else c--;
            if(r<0 or c<0) { flag = 0; break; }
            cur ^= grid[r][c];
        }
        if(!flag) continue;
        debug(r,c,cur)
        if(c) ans += ways[r][c-1][cur^k];
        if(r) ans += ways[r-1][c][cur^k];
    }
    putn(ans)
}
