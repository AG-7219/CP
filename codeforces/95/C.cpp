...
void TATAKAE(int T)
{
    ...
    auto dijikstra = [&](int src)
    {
        ...
        while(!st.empty())
        {
            ...
            {
                if(dist[src][child] > dist[src][node] + wt)
                {
                    st.erase({dist[src][child],child});
                    dist[src][child] = dist[src][node]+wt;
                    st.emplace(dist[src][child], child);
                }
            }
        }
    };
    rep(i,1,n+1) dijikstra(i);
    ...
    while(!st.empty())
    {
        ...
        {
            if(dist[node][child] <= taxies[node].first and ans[child] > ans[node]+taxies[node].second)
            {
                st.erase({ans[child],child});
                ans[child] = ans[node] + taxies[node].second;
                st.emplace(ans[child],child);
            }
        }
    }
    if(ans[y] == oo) ans[y] = -1;
