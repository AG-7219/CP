...
void TATAKAE(int T)
{
    ...
    rep(i,1,3005)
    {
        //add fruits for today
        ms.emplace(i,fruits[i]);

        //remove fruits that withered already
        if(i-2>=1 and fruits[i-2]) ms.erase({i-2,fruits[i-2]});

        //harvest fruits
        int cur = 0;
        while(ms.size() and cur<v)
        {
            auto [day, gain] = *ms.begin();
            ms.erase(ms.begin());

            int take = min(gain,v-cur);
            cur += take;
            gain -= take;

            if(gain) ms.emplace(day,gain);
            fruits[day] = gain;
        }
        ans += cur; 
    }
