...
void TATAKAE(int T)
{
    ...
    vector<map<int,int>> sack(n+1);
    function<int(int,int,int)> dfs = [&](int node, int par, int depth)
    {
        sack[node][depth]++;
        int mx = 1, arg_mx = depth;
        for(int child:adj[node])
        {
            if(child!=par)
            {
                int candidate = dfs(child,node,depth+1);
                if(sack[child].size() > sack[node].size()) swap(sack[node],sack[child]);
                for(auto& [key,value]:sack[child])
                {
                    if(amax(mx, sack[node][key] += value)) arg_mx = key;
                    else if(mx == sack[node][key]) amin(arg_mx,key);
                }
                if(amax(mx,sack[node][candidate])) arg_mx = candidate;
                else if(mx == sack[node][candidate]) amin(arg_mx,candidate);
            }
        }
        ans[node] = arg_mx - depth;
        return arg_mx;
    };
    dfs(1,-1,0);
    logarr(ans,1,n+1)
