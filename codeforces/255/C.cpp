...
void solve(int T)
{
    ...
    vector<int> aux(1+all(v));
    sort(all(aux));
    uniq(aux);

    vector<vector<int>> dp(aux.size(), vector<int> (aux.size(),0));

    rep(i,1,n+1)
    {
        int ind = lower_bound(all(aux),v[i]) - aux.begin();
        rep(j,0,aux.size()) if(ind!=j and (dp[j][ind]&1)) dp[j][ind]++;
        rep(j,0,aux.size()) if(ind!=j and (dp[ind][j]%2 == 0)) dp[ind][j]++;
        dp[ind][ind]++;
    }

    int ans = 0;
    rep(i,0,aux.size()) rep(j,0,aux.size()) amax(ans,dp[i][j]);
    cout<<ans<<"\n";
}
