...
{
    get(string,s,t)
    vector<int> pre(t.length()), suf(t.length());
    int j = 0;
    rep(i,0,t.length())
    {
        while(j<s.length() and s[j]!=t[i]) j++;
        pre[i] = j++;
    }
    j = s.length()-1;
    repr(i,t.length()-1,-1)
    {
        while(j>=0 and s[j]!=t[i]) j--;
        suf[i] = j--;
    }
    debug(pre,suf)
    vector<vector<pii>> pairs(26,vector<pii> ());
    rep(i,0,t.length()) pairs[t[i]-'a'].pb(pre[i],suf[i]);
    rep(i,0,26) sort(all(pairs[i]),greater<>());
    vector<vector<int>> aux(26), mx(26);
    rep(i,0,26) 
    {
        rep(j,0,pairs[i].size()) aux[i].push_back(pairs[i][j].first), mx[i].push_back(pairs[i][j].second); 
        partial_sum(rall(mx[i]),mx[i].rbegin(),g(l,r,max(l,r)));
    }
    debug(aux,mx)
    rep(i,0,s.length())
    {
        int ind = lower_bound(all(aux[s[i]-'a']),i,greater<>()) - aux[s[i]-'a'].begin();
        ind = min(ind,(int)mx[s[i] - 'a'].size()-1);
        debug(ind,i,mx[s[i]-'a'][ind])
        if(ind==-1 or mx[s[i] - 'a'][ind] < i) putr("No")
    }
    putn("Yes")
}
