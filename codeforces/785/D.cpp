...
#ifdef DEBUG
#include "/storage/emulated/0/CP/Debug/debug.hpp"
#else
#define debug(args...) 0;
...
void TATAKAE(int T)
{
	vector<int> facts(N,1), inv(N,1);
	rep(i,2,N) facts[i] = (facts[i-1] * i)%mod;
	rep(i,0,N) inv[i] = expm(facts[i], mod-2);
	...
	string s; cin>>s;
	int o = 0, c = count(all(s),')'), ans = 0;
	for(int i=0; i<s.length();i++)
	{
		if(s[i]=='(')
		{
			o++;
			ans = (ans + nCr(o+c-1, o))%mod;
		}
		else c--;
		debug(i,ans,o,c)
	}
