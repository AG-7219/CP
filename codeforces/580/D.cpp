...
void solve(int T)
{
    ...
    get(int,n,m,k)
    vector<int> a(n,0); rep(i,0,n) cin>>a[i];
    ...
    rep(mask,0,1<<n)
    {
        rep(last,0,n)
        {
            rep(cur,0,n)
            {
                if(((mask>>last)&1) and (((mask>>cur)&1) == 0))
                {
                    amax(dp[cur][mask|(1<<cur)], dp[last][mask] + a[cur] + bonus[last][cur]);
                }
            }
        }
    }
    ...
    int ans = -oo;
    rep(mask,0,1<<n) rep(last,0,n) if(bpc(mask) == m) amax(ans,dp[last][mask]);
    putn(ans)
}
