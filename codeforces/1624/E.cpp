...
{   
    get(int,n,m)
    vector<string> vs(n+1); cin>>vs;
    map<string,tuple<int,int,int>> mp;
    rep(i,1,n+1)
    {
        rep(j,0,m-1)
        {
            if(mp.contains(vs[i].substr(j,2))) continue;
            else mp[vs[i].substr(j,2)] = {j+1,j+2,i};
        }
    }
    rep(i,1,n+1)
    {
        rep(j,0,m-2)
        {
            if(mp.contains(vs[i].substr(j,3))) continue;
            else mp[vs[i].substr(j,3)] = {j+1,j+3,i};
        }
    }
    get(string,s)
    s.insert(s.begin(), ' ');
    vector<bool> dp(s.length(),false);
    vector<int> back(s.length(),-1);
    dp[0] = true;
    rep(i,2,s.length())
    {
        if(mp.contains(s.substr(i-1,2)) and dp[i-2]) dp[i] = true, back[i] = i-2;
        if(i>=3 and mp.contains(s.substr(i-2,3)) and dp[i-3]) dp[i] = true, back[i] = i-3;
    }
    if(!dp.back()) putr(-1)
    ...
    while(back[cur] != -1)
    {
        if(back[cur] == cur-2) ans.pb(mp[s.substr(cur-1,2)]);
        else ans.pb(mp[s.substr(cur-2,3)]);
        cur = back[cur];
        log(cur,back[cur])
    }
    reverse(all(ans));
    putn(ans.size())
    for(auto& [l,r,i]:ans) putn(l,r,i)
}
