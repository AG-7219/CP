...
void TATAKAE(int T)
{
    ...
    cin>>n>>p>>t;
    vector<ld> dp(n+1,0);
    //dp[i] denotes prob of i people standing on elevator at some instant 
    
    //initially no person stands on the elevator
    dp[0] = 1;
    ...
    rep(i,1,t+1)
    {
        ...
        {
            //at time i seconds, j people stand on the elevator

            //either they were already standing and the person didnt move
            if(j!=n) dp[j] *= (1-p);

            //exactly one person got on the elevator right now
            if(j) dp[j] += dp[j-1]*(p);
        }
        debug(i,dp)
    }
