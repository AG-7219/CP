...
{   
    ...
    rep(i,2,n+1) if(a[i-1] > a[i]) left[i] += left[i-1];
    repr(i,n-1,0) if(a[i+1] > a[i]) right[i] += right[i+1];
    int l = 1, r = n, last = 0;
    string ans = "";
    while(r>=l)
    {
        ...
        else if((a[l] > a[r] and a[r]>last) or a[l] <= last)
        {
            ans += "R";
            last = a[r--];
        }
        ...
        {
            if(a[r] > last and right[l] < left[r]) ans += string(left[r],'R');
            else if(a[l] > last) ans += string(right[l],'L');
            break ;
        }
    }
    putn(ans.length())
