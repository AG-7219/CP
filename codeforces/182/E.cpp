...
{
    get(int,n,l)
    vector<pii> tiles(n+1); cin>>tiles;
    vector dp1(l+1, vector (n+1,0LL));
    vector dp2(l+1, vector (n+1,0LL));

    rep(i,1,n+1)
    {
        if(tiles[i].first <=l) dp1[tiles[i].first][i] = 1;
        if(tiles[i].first != tiles[i].second and tiles[i].second<=l) dp2[tiles[i].second][i] = 1;
    }
    ...
    rep(i,1,l+1)
    {
        for(int prev=1; prev<=n; prev++)
        {
            for(int cur=1; cur<=n; cur++)
            {
                ...
                auto [cur_a,cur_b] = tiles[cur];
                auto [prev_a,prev_b] = tiles[prev];
                if(cur_a == prev_b and i+cur_a <= l) dp1[i + cur_a][cur] = (dp1[i + cur_a][cur] + dp1[i][prev])%mod;
                if(cur_a == prev_a and i+cur_a <= l) dp1[i + cur_a][cur] = (dp1[i + cur_a][cur] + dp2[i][prev])%mod;
                if(cur_a != cur_b)
                {
                    if(cur_b == prev_b and i+cur_b <= l) dp2[i + cur_b][cur] = (dp2[i + cur_b][cur] + dp1[i][prev])%mod;
                    if(cur_b == prev_a and i+cur_b <= l) dp2[i + cur_b][cur] = (dp2[i + cur_b][cur] + dp2[i][prev])%mod;
                }
            }
        }
    }
    ...
    int ans = 0;
    rep(i,1,n+1) ans = (ans + dp1[l][i])%mod, ans = (ans + dp2[l][i])%mod;
    putn(ans)
}
