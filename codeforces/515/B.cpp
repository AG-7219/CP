...
const int N = 10001, M = 998244353, mod = 1000000007, MX = INT64_MAX, MN = INT64_MIN, oo = 1e18;
...
void merge(vector<int>& size, vector<int>& parent, vector<bool>& is_happy, int a, int b)
{
    ...
    parent[b]=a;
    size[a]+=size[b];
    is_happy[a] = is_happy[a] or is_happy[b];
}

void solve(int T)
{
    get(int,n,m)
    vector<int> sizes(n+m,1), parent(n+m);
    rep(i,0,n+m) parent[i] = i;
    vector<bool> is_happy(n+m,false);
    get(int,k)
    while(k--)
    {
        get(int,a)
    ...
    rep(i,1,N)
    {
        merge(sizes,parent,is_happy,i%n,n + i%m);
    }    
    rep(i,0,n+m)
    {
        if(!is_happy[find(parent,i)]) putr("No")
    }
    putn("Yes")
}
