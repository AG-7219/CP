...
void TATAKAE(int T)
{
    ...
    vector<int> next(s.length(),n-1), prev(s.length(),0);
    int last = n-1;
    for(int i=n-1; i>=0; i--)
    {
        next[i] = last;
        if(special[s[i] - 'a']) last = i;
    }
    last = 0;
    ...
    for(int i=0; i<s.length(); i++)
    {
        if(special[s[i] - 'a'])
        {
            int available = i - prev[i] + rem;
            int time = next[i] - i;
            if(next[i] == n-1) time = oo;
            amax(ans,min(available,time));
            rem = max(0LL,available - time);
            debug(available,time,rem,ans,i)
        }
    }
    putn(ans)
}
