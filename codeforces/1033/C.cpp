...
void solve(int T)
{
    ...
    rep(i,1,n+1)
    {
        for(int j=i; j<=n; j+=i)
        {
            int tar1 = pos[i] + j;
            int tar2 = pos[i] - j;
            if(tar1 >= 1 and tar1<=n and v[tar1] > i) adj[pos[i]].pb(tar1);
            if(tar2 >= 1 and tar2<=n and v[tar2] > i) adj[pos[i]].pb(tar2);
        }
    }
    // rep(i,1,n+1)
    // {
    //     cout<<"adj of "<<i<<" is: ";
    //     logarr(adj[i],0,adj[i].size())
    // }
    vector<int> in(n+1,0);
    rep(i,1,n+1) for(int child:adj[i]) in[child]++;
    queue<int> q;
    ...
    for(int node:topo)
    {
        for(int child:adj[node])
        {
            if(ans[child] == false) ans[node] = true;
        }
    }
    rep(i,1,n+1)
    {
