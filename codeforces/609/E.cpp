...
void solve(int T)
{
    ...
    int l = __lg(bit_ceil(n+1ULL));
    vector<int> depth(n+1,0);
    vector<vector<int>> up(n+1,vector<int> (l,1)), mx(n+1,vector<int> (l,0));
        ...
        for(auto& [child,w]:adj[node])
        {
            if(child!=up[node][0])
            {
                ...
                rep(i,1,l)
                {
                    ...
                    mx[child][i] = max(mx[child][i-1],mx[up[child][i-1]][i-1]);
                }
    ...
    auto lca = [&](int u, int v)
    {
        ...
        return make_pair(up[u][0],max({ans,mx[u][0],mx[v][0]}));
    };

    for(auto& [w,u,v]:edges)
    {
        auto [_,weight] = lca(u,v);
        putn(cost - weight + w)
    }
}
