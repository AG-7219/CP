...
void solve(int T)
{
    ...
    for(char c:good) is_good[c-'a'] = true;
    get(string,s)

    auto isvalid = [&](string a)
    {
        for(char c:a) if(is_good[c-'a']) return false;
        return true;
    };
    get(int,m)
    while(m--)
    {
        ...
        while(i<s.length() and j<t.length())
        {
            if(s[i] == '?')
            {
                if(!is_good[t[j]-'a']) { flag = false; break; }
            }
            else if(s[i] == '*')
            {
                int del = (t.length() - j) - (s.length() - (i+1));
                if(del < 0) { flag = false; break; }
                if(!isvalid(t.substr(j,del))) { flag = false; break; }
                j += del-1;
            }
            else 
            {
                if(s[i] != t[j]) { flag = false; break; }
            }
            i++; j++;
        }
        if(j<t.length()) flag = false;
        if(i<s.length() and (i!=s.length()-1 or s[i]!='*')) flag = false;
        cout<< ((flag) ? "YES\n" : "NO\n");
    }
}
