...
void solve(int T)
{
    ...
    //dp1[node] stores the max profit when starting at node and ending in subtree
    //dp2[node] stores the max profit when starting(and ending) at some node in the subtree of node and also passing through the node
    ...
    function<void(int,int)> dfs = [&](int node, int par)
    {
        ...
        for(auto& [child,weight]:adj[node])
        {
            ...
            {
                dfs(child,node);
                amax(dp1[node],dp1[child]+a[node]-weight);
                pq.push(dp1[child]-weight);
            }
        }  
        if(pq.size() >= 2)
        {
    ...
    dfs(1,-1);
    putn(max(*maxv(dp1,1,n+1),*maxv(dp2,1,n+1)))
}
