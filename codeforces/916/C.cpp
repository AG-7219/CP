...
const int N = 1000001, M = 998244353, mod = 1000000007, MX = INT64_MAX, MN = INT64_MIN, oo = 1e18;
...
void TATAKAE(int T)
{
    ...
    vector<int> primes; for(int i=2; i<N; i++) if(!sieve[i]) primes.push_back(i);
    int p = *lower_bound(all(primes),n-1);
    vector<tuple<int,int,int>> ans;
    for(int i=1; i+1<n; i++) ans.pb(i,i+1,1), m--;
    ans.pb(n-1,n,1+p-(n-1)); m--;
    for(int i=1; i<=n and m; i++) for(int nbr = i+2; nbr<=n and m; nbr++) ans.pb(i,nbr,p+1), m--;
    putn(p,p);
    for(auto& [u,v,w]:ans) putn(u,v,w)
