...
void solve(int T)
{
    ...
    vector<vector<vector<int>>> dp(s.length(),vector<vector<int>>(k+1,vector<int> (26,-oo)));
    rep(i,0,26)
    {
        if(s.front() -'a' == i) continue;
        if(k>=1) dp[0][1][i] = 0;
    }
    dp[0][0][s.front() - 'a'] = 0;
    rep(i,1,s.length())
    {
        rep(j,0,26) amax(dp[i][0][s[i]-'a'], dp[i-1][0][j] + bonus[j][s[i]-'a']);
        rep(j,1,k+1)
        {
            rep(ch,0,26)
            {
                rep(prev,0,26)
                {
                    if(ch == s[i]-'a') amax(dp[i][j][ch], dp[i-1][j][prev] + bonus[prev][ch]);
                    else amax(dp[i][j][ch],dp[i-1][j-1][prev] + bonus[prev][ch]);
                }
            }
        }
    }
    int ans = -oo;
    rep(i,0,k+1) rep(j,0,26) amax(ans,dp[s.length()-1][i][j]);
    putn(ans)
}
