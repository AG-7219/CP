...
const int N = 10000, M = 998244353, mod = 1000000007, MX = INT64_MAX, MN = INT64_MIN, oo = 1e18;
...
void TATAKAE(int T)
{
    ...
    while(n--)
    {
        vector<int> new_dp(2*N+1,0);
        get(int,x)
        rep(i,0,dp.size())  if(i+x < new_dp.size()) new_dp[i+x] = (new_dp[i+x] + dp[i])%mod;
        rep(i,x,dp.size()) new_dp[i-x] = (new_dp[i-x] + dp[i])%mod;
        new_dp[N + x] = (new_dp[N+x] + 1)%mod;
        new_dp[N - x] = (new_dp[N-x] + 1)%mod;
