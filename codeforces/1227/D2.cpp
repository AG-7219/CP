...
{
    get(int,n)
    vector<int> a(n), o(n); for(int& val:a) cin>>val;
    iota(all(o),0);
    sort(all(o),[&](const int& l, const int& r){
        if(a[l] == a[r]) return r < l;
        return a[r] > a[l];
    });
    oset(int) os;
    get(int,m)
    vector<int> k(m), pos(m), o_q(m), ans(m,-1); for(int i=0; i<m; i++) cin>>k[i]>>pos[i];   
    iota(all(o_q),0); sort_by(all(o_q), k[l] < k[r]);
    for(int qi:o_q)
    {
        while(os.size() < k[qi]) os.insert(o.back()), o.pop_back();
        ans[qi] = a[*os.find_by_order(pos[qi]-1)];
    }
