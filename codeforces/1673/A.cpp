...
void TATAKAE(int T)
{
    ...
    {
        int c1 = 0, c2 = 0;
        rep(i,0,s.length()-1) c1 += s[i] - 'a' + 1;
        rep(i,1,s.length()) c2 += s[i] - 'a' + 1;
        int alice = max(c1,c2);
        int bob = min(s[0]-'a'+1,s.back()-'a'+1);
        if(alice > bob) putn("Alice",alice-bob)
        else putn("Bob",bob-alice)
    }
    else 
    {
        int ans = 0;
        rep(i,0,s.length()) ans += s[i] - 'a'+1;
        putn("Alice",ans)
    }
}
