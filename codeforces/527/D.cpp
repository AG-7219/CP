...
//lets analyse for (i,j,k)
// i and j satisfy this => xi-xj >= wi + wj
// j and k satisfy this => xj-xk >= wj + wk
// add both xi-xk >= wi+2wj+wk >= wi + wk. Hence i and k satisfy this.
...
{
    get(int,n)
    vector<int> x(n),w(n),o(n), dif(n);
    for(int i=0; i<n; i++) cin>>x[i]>>w[i], o[i] = i, dif[i] = x[i] + w[i];
    sort_by(all(o), x[l] < x[r]);
    sort(all(dif));
    uniq(dif);
    segtree root(dif.size());
    for(int i=0; i<n; i++)
    {
        int cur = x[o[i]] - w[o[i]];
        int index = upper_bound(all(dif),cur) - dif.begin();
        int mx = root.query(0,index);
        root.update(lower_bound(all(dif),x[o[i]] + w[o[i]]) - dif.begin(), mx+1);
    }
    putn(root.query(0,dif.size()))
}
