...
void TATAKAE(int T)
{
    get(int,n)
    int sum = 0;    
    vector<vector<int>> digits(3,vector<int> ());
    for(int i=0; i<n; i++)
    {
        int x; cin>>x; sum += x;
        digits[x%3].push_back(x);
    }
    rep(i,0,3) sort(rall(digits[i]));
    if(digits[0].empty() or digits[0].back() != 0) putr(-1)
    if(sum%3)
    {
        if(digits[sum%3].size()) digits[sum%3].pop_back();
        else 
        {
            int other = 3 - sum%3;
            assert(digits[other].size() >= 2);
            digits[other].pop_back();
            digits[other].pop_back();
        }
    }
    vector<int> temp;
    for(int i=0; i<3; i++) for(int& digit:digits[i]) temp.push_back(digit);
    sort(rall(temp));
    string ans = "";
    for(int& digit:temp) ans += to_string(digit);
    reverse(all(ans));
